...
import { LOCALE_COOKIE } from "@ohmyagentteam/core/i18n";
import {
  OMAT_LOCALE_HEADER,
...
// Forward the resolved locale to RSC layouts via the `x-ohmyagentteam-locale`
...
function nextWithLocale(req: NextRequest): NextResponse {
  ...
  headers.set(OMAT_LOCALE_HEADER, resolveLocale(req));
...
export function proxy(req: NextRequest) {
  ...
  const hasSession = req.cookies.has("omat_logged_in");
