...
export default ({ config }: ConfigContext): ExpoConfig => {
  ...
  return {
    ...
    name: isProd
      ? "OhMyAgentTeam"
      : isStaging
        ? "OhMyAgentTeam (Staging)"
        : "OhMyAgentTeam (Dev)",
    slug: "ohmyagentteam-mobile",
    version: "0.1.0",
    orientation: "portrait",
    userInterfaceStyle: "automatic",
    scheme: ["ohmyagentteam", "ohmyagentteam"],
    ...
    ios: {
      ...
      // OhMyAgentTeam Apple Developer team (and external users self-building a
      ...
      bundleIdentifier: isProd
        ? (process.env.EXPO_BUNDLE_IDENTIFIER_PROD ?? "ai.ohmyagentteam.mobile")
        : isStaging
          ? "ai.ohmyagentteam.mobile.staging"
          : (process.env.EXPO_BUNDLE_IDENTIFIER_DEV ?? "ai.ohmyagentteam.mobile.dev"),
    },
    plugins: [
      "expo-router",
      "expo-secure-store",
      ...
      [
        "expo-image-picker",
        {
          ...
          photosPermission:
            "Allow OhMyAgentTeam to access your photos to attach images to work items and comments.",
