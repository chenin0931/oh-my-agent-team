...
description: Common issues when self-hosting OhMyAgentTeam — symptoms, causes, how to diagnose, how to fix.
---

import { Callout } from "fumadocs-ui/components/callout";

Look up issues by symptom. Each entry gives you **symptom / likely causes / how to diagnose / how to fix**. If your situation isn't listed, open an issue on [GitHub](https://github.com/chenin0931/oh-my-agent-team/issues).

## Daemon can't connect to the server

**Symptom**: [`omat daemon`](/cli)'s `status` command shows `offline` or `connection refused`; the server logs show no `/api/daemon/register` or `/api/daemon/heartbeat` requests. For how the daemon mechanism works, see [Daemon and runtimes](/daemon-runtimes).

**Likely causes**:

1. **`OMAT_SERVER_URL` points at the wrong address** — default is `ws://localhost:8080/ws`; self-host must change it to your server address
2. **Network / firewall blocking** — the daemon and server aren't on the same network, or outbound traffic is blocked
3. **Token expired or invalid** — you never ran `omat login`, or the PAT was revoked
...
```bash
omat daemon logs --lines 100    # look for daemon-side errors
echo $OMAT_SERVER_URL          # confirm the address is set
...
cat ~/.ohmyagentteam/config.json        # verify api_token exists
omat workspace list            # confirm you're a member of the target workspace
```

**How to fix**: address each cause above. The two most common fixes are **changing `OMAT_SERVER_URL` and restarting the daemon** (`omat daemon restart`) and **signing in again** (`ohmyagentteam logout && omat login`).

## Tasks stuck in `queued`

**Symptom**: after assigning an issue to an agent, the issue status flips to `in_progress` immediately, but a long time passes with no sign of agent execution on the page; `omat daemon status` shows the daemon `online`.
...
```bash
omat daemon status --output json       # runtime list + last_seen_at
omat agent list                         # check agent archived state
omat issue show <issue-id>             # inspect task history
```
...
- Concurrency full → wait for running tasks to finish, or `omat agent update <id> --max-concurrent-tasks 10` to raise the ceiling
- Same-issue serialization → wait for the previous task to finish, or reassign to a different agent
- Agent archived → `omat agent restore <id>`
- Runtime not registered → `omat daemon restart`, and the daemon will re-register
...
- Wrong origin → set `FRONTEND_ORIGIN=https://ohmyagentteam.yourdomain.com` in the server's `.env` (or comma-separated `CORS_ALLOWED_ORIGINS`) and restart the server
...
| `smtp RCPT TO <addr>: 550 5.7.1 Unable to relay` | The relay's receive connector doesn't allow your subnet to relay to external recipients (most common for anonymous internal relays talking to outside domains) | Either restrict invites to internal recipients, or add the OhMyAgentTeam host's subnet to the Exchange "Anonymous Users → Relay" permission list |
...
- `Unable to relay` → either restrict to internal recipients or grant the OhMyAgentTeam host's IP relay permission on the Exchange receive connector
...
1. **`OMAT_DEV_VERIFICATION_CODE` is empty** — fixed codes are disabled by default
...
```bash
cat .env | grep -E 'APP_ENV|OMAT_DEV_VERIFICATION_CODE'
docker exec <container> env | grep -E 'APP_ENV|OMAT_DEV_VERIFICATION_CODE'
```
...
- In production, leave `OMAT_DEV_VERIFICATION_CODE` empty — configure Resend and use real codes
- For local development or internal testing, either copy the generated code from server logs or set `APP_ENV=development` plus `OMAT_DEV_VERIFICATION_CODE=888888` — never enable a fixed code on a public instance (see [Sign-in and signup configuration → Fixed local testing codes](/auth-setup#fixed-local-testing-codes))
...
2. **`pg_cron` is configured for compatibility but pointing at the wrong database** — `pg_cron.database_name` defaults to `postgres`; if your OhMyAgentTeam database has a different name, the scheduled job never sees `rollup_task_usage_hourly()`. The in-process scheduler does not depend on this, but if you removed the in-process scheduler and rely on `pg_cron`, the DB name must match.
...
**Symptom**: `ohmyagentteam server` or `omat daemon start` fails with `address already in use`.
...
|---|---|---|
| **Daemon** | `~/.ohmyagentteam/daemon.log` (background mode) or foreground stdout | `omat daemon logs -f --lines 100` |
| **Server (Docker)** | Container stdout | `docker logs -f <container>` |
| **Server (systemd)** | journal | `journalctl -u ohmyagentteam-server -f` |
| **Frontend (dev)** | Terminal running `pnpm dev` | Read directly |
| **Frontend (browser)** | DevTools → Console | Press `F12` |

For more detailed daemon logs, move it from background to foreground: `omat daemon stop && omat daemon start --foreground`.
