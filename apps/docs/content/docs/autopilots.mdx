...
The OhMyAgentTeam server scans for due triggers every **30 seconds** — **the actual fire time can lag by up to 30 seconds**, not down to the second. If the server is restarted across a fire time, it catches up missed triggers on startup (nothing is lost, but they fire right away).
...
on the autopilot detail page; OhMyAgentTeam generates a unique URL of the shape:

```
https://<your-ohmyagentteam-host>/api/webhooks/autopilots/awt_…
```

POST any JSON to that URL — OhMyAgentTeam records a run with `source = webhook`,
...
…or any JSON object/array. OhMyAgentTeam normalizes it into an internal envelope:
...
When you don't provide an `event` field, OhMyAgentTeam infers it from common
...
comma-separated **actions** list. OhMyAgentTeam allows a webhook if **any**
...
OhMyAgentTeam derives the `event` name and `action` from the inbound request
...
**2. Headers.** When no body envelope is present, OhMyAgentTeam reads the
...
present, OhMyAgentTeam falls back to top-level body string fields in this
...
**Action candidates, in full.** Once the event is determined, OhMyAgentTeam
...
- **Rotate it if it leaks** — click "Rotate URL" on the trigger row, or run
  `omat autopilot trigger-rotate-url <autopilot-id> <trigger-id>`. The
...
OhMyAgentTeam returns `200 OK` with a `status` field for normal no-op outcomes so
...
When `OMAT_PUBLIC_URL` is set on the server (e.g. `https://ohmyagentteam.example.com`),
...
custom self-hosted reverse proxies. OhMyAgentTeam deliberately does not derive
