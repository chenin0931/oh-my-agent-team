...
Autopilots 让 [智能体](/agents) **按调度自动开工**——配好 cron 和时区，到点 OhMyAgentTeam 自己派发 [`task`](/tasks)，不需要你每次触发。适合定期巡检、周期性报告、凌晨跑的清理任务这类"standing order"场景。和前三种触发方式（[分配](/assigning-issues) / [@ 提及](/mentioning-agents) / [对话](/chat) 都是你主动喊一声）相比，Autopilots 的核心差别是**时间驱动**。
...
OhMyAgentTeam 服务器每 **30 秒**扫一次到期的触发器——**触发时刻最多延迟 30 秒**，不是秒级精准。服务器重启时如果恰好错过触发点，启动时会补扫漏掉的触发（不会丢触发，但会立刻补跑）。
...
触发器，OhMyAgentTeam 会生成一个唯一的 URL：

```
https://<你的 OhMyAgentTeam host>/api/webhooks/autopilots/awt_…
```

向这个 URL POST 任意 JSON——OhMyAgentTeam 会记录一条 `source = webhook` 的
...
也可以直接发任意 JSON 对象 / 数组。OhMyAgentTeam 会规范化为内部封装：
...
不带 `event` 字段时，OhMyAgentTeam 会按以下顺序从常见 header 和 body 字段
...
OhMyAgentTeam 按下面的顺序从入站请求里推断 `event` 和 `action`，**先命中先用**。
...
- **泄漏后立即重新生成**——在触发器上点"重新生成 URL"，或运行
  `omat autopilot trigger-rotate-url <autopilot-id> <trigger-id>`。
  旧 URL 立即失效。
...
服务端设置 `OMAT_PUBLIC_URL`（例如 `https://ohmyagentteam.example.com`）后，
触发器响应里会带绝对的 `webhook_url`，UI 直接显示可复制的 URL。没设
时 UI 会用客户端的 API origin 拼出 URL——desktop 和同源 web 没问题，
但自定义反向代理就不行了。OhMyAgentTeam **故意不**从 `Host` /
