---
title: CLI Reference
description: Complete command reference for the OhMyAgentTeam CLI and agent daemon.
---
...
| Agent timeout | `--agent-timeout` | `OMAT_AGENT_TIMEOUT` | `0`（不限制，由看门狗兜底）|
...
| `OMAT_CURSOR_MODEL` | Override the Cursor model used |
...
When connecting to a self-hosted OhMyAgentTeam instance, point the CLI to your server before logging in:

```bash
export OMAT_APP_URL=https://app.example.com
export OMAT_SERVER_URL=wss://api.example.com/ws
...
omat config set server_url wss://api.example.com/ws
```
...
```bash
omat workspace watch <workspace-id>
omat workspace unwatch <workspace-id>
```
...
按邮箱邀请成员加入 workspace。被邀请人会收到一条待接受（pending）的邀请，接受后才真正加入——不是即时添加。workspace 参数可选，接受完整 UUID、slug 或短 UUID 前缀（≥4 位十六进制），省略时用当前默认 workspace（`--workspace-id` / `OMAT_WORKSPACE_ID` / profile 默认）。

```bash
omat workspace member invite alice@example.com
omat workspace member invite alice@example.com <workspace-id> --role admin
omat workspace member invite alice@example.com --output json
```
...
```bash
omat workspace update <workspace-id> --name "Acme Eng"
omat workspace update <workspace-id> \
...
```bash
cat <<'CTX' | omat workspace update <workspace-id> --context-stdin
...
```bash
omat issue get MUL-123
omat issue get <uuid>
omat issue get <id> --output json
```

`<id>` 同时接受 issue key（`omat issue list` 表格里直接显示，例如 `MUL-123`）和完整 UUID（给 `list` 加 `--full-id` 可显示）。同样的规则适用于下面 `update` / `assign` / `status` / `comment` / `subscriber` / `runs` 等接受 `<id>` 的命令。
...
Flags: `--title` (required), `--description`, `--status`, `--priority`, `--assignee` / `--assignee-id`, `--parent`, `--project`, `--due-date`. 脚本里如果已经拿到了 UUID（例如来自 `omat workspace member list --output json`），传 `--assignee-id <uuid>`（与 `--assignee` 互斥）以精确锁定。
