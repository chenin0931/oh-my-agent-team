...
description: Deploy OhMyAgentTeam on your own infrastructure.
---

## Architecture Overview

OhMyAgentTeam has three components:
...
Each user who wants to run AI agents locally also installs the **`ohmyagentteam` CLI** and runs the **agent daemon** on their own machine.
...
<Callout>
Alternatively, configure step by step: `omat config set server_url http://localhost:8080 && omat config set app_url http://localhost:3000 && omat login && omat daemon start`
</Callout>
...
Full reference (audit table semantics, advisory lock 4246, the standalone backfill command, flag descriptions, the migration auto-hook) lives in [`SELF_HOSTING_ADVANCED.md → Usage Dashboard Rollup`](https://github.com/chenin0931/oh-my-agent-team/blob/main/SELF_HOSTING_ADVANCED.md#usage-dashboard-rollup).
...
|----------|-------------|---------|
| `DATABASE_URL` | PostgreSQL connection string. Keep the password segment in sync with `POSTGRES_PASSWORD`. | `postgres://ohmyagentteam:<postgres-password>@localhost:5432/ohmyagentteam?sslmode=disable` |
...
OhMyAgentTeam uses email-based magic link authentication via [Resend](https://resend.com).
...
| `OMAT_CODEBUDDY_PATH` | Custom path to the `codebuddy` binary |
| `OMAT_CODEBUDDY_MODEL` | Override the CodeBuddy model used |
| `OMAT_ANTIGRAVITY_PATH` | Custom path to the `agy` binary |
| `OMAT_ANTIGRAVITY_MODEL` | Override the Antigravity model used |
...
| `OMAT_TRAECLI_MODEL` | Override the Trae CLI model used |
...
This starts a `pgvector/pgvector:pg17` container on port 5432 with default credentials (`ohmyagentteam`/`ohmyagentteam`).
