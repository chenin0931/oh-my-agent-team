...
describe("runtime config", () => {
  it("uses cloud defaults without a desktop.json file", () => {
    expect(DEFAULT_RUNTIME_CONFIG).toEqual({
      schemaVersion: 1,
      apiUrl: "https://api.ohmyagentteam.com",
      wsUrl: "wss://api.ohmyagentteam.com/ws",
  ...
  it("strips the leading api. label when deriving appUrl", () => {
    expect(
      parseRuntimeConfig(
        JSON.stringify({ schemaVersion: 1, apiUrl: "https://api.ohmyagentteam.com" }),
      ),
    ).toEqual({
      schemaVersion: 1,
      apiUrl: "https://api.ohmyagentteam.com",
  ...
  it("rejects non-http api schemes", () => {
    expect(() =>
      parseRuntimeConfig(JSON.stringify({ schemaVersion: 1, apiUrl: "file:///tmp/omat" })),
  ...
  it("derives dev appUrl by stripping the leading api. label", () => {
    ...
    // public web host, not the api host. OhMyAgentTeam's convention exposes the
    // api at `api.<web-host>`, so stripping the leading label gives the
    // right web origin without a separate VITE_APP_URL.
    expect(
      runtimeConfigFromDevEnv({ apiUrl: "https://api.test.ohmyagentteam.com" }),
    ).toEqual({
      schemaVersion: 1,
      apiUrl: "https://api.test.ohmyagentteam.com",
      wsUrl: "wss://api.test.ohmyagentteam.com/ws",
      appUrl: "https://test.ohmyagentteam.com",
    });
  });
  ...
  it("dev VITE_APP_URL still wins over apiUrl-derived value", () => {
    expect(
      runtimeConfigFromDevEnv({
        apiUrl: "https://api.test.ohmyagentteam.com",
        appUrl: "https://staging.ohmyagentteam.com",
      }),
    ).toEqual({
      schemaVersion: 1,
      apiUrl: "https://api.test.ohmyagentteam.com",
