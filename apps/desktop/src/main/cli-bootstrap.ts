...
// Desktop prefers the bundled `ohmyagentteam` CLI shipped inside the app for
...
const GITHUB_LATEST_BASE =
  "https://github.com/chenin0931/oh-my-agent-team/releases/latest/download";

function binaryName(): string {
  return process.platform === "win32" ? "omat.exe" : "ohmyagentteam";
}
...
async function installFresh(): Promise<string> {
  ...
  const workDir = join(tmpdir(), `omat-cli-${Date.now()}`);
...
/**
 * Returns the path to a usable `ohmyagentteam` binary. If one is already present at
