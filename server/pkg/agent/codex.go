...
const (
	omatCodexMcpBeginMarker = "# BEGIN ohmyagentteam-managed mcp_servers (do not edit; regenerated by daemon)"
	omatCodexMcpEndMarker   = "# END ohmyagentteam-managed mcp_servers"
)

var codexMcpBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(omatCodexMcpBeginMarker) +
		`.*?^` + regexp.QuoteMeta(omatCodexMcpEndMarker) + `\n*`)
...
func ensureCodexMcpConfig(configPath string, mcpConfig json.RawMessage, logger *slog.Logger) error {
	...
	if managed {
		...
		if block == "" {
			block = omatCodexMcpBeginMarker + "\n" + omatCodexMcpEndMarker + "\n"
		}
		if stripped == "" {
			updated = block
		} else {
...
func renderCodexMcpServersBlock(raw json.RawMessage) (string, bool, error) {
	...
	sb.WriteString(omatCodexMcpBeginMarker)
	sb.WriteString("\n")
	...
	sb.WriteString(omatCodexMcpEndMarker)
	sb.WriteString("\n")
...
func normalizeCodexMcpServerConfig(server map[string]any) map[string]any {
	if !isCodexRemoteMcpServer(server) {
		...
		for k, v := range server {
			if isOhMyAgentTeamMcpSelectorKey(k) {
				continue
			}
			normalized[k] = v
		}
	...
	for k, v := range server {
		switch {
		case isOhMyAgentTeamMcpSelectorKey(k):
			continue
		case k == "type":
			continue
		case k == "headers":
...
func isOhMyAgentTeamMcpSelectorKey(k string) bool {
	switch k {
	...
	go func() {
		...
		_, err := c.request(runCtx, "initialize", map[string]any{
			"clientInfo": map[string]any{
				"name":    "ohmyagentteam-agent-sdk",
				"title":   "OhMyAgentTeam Agent SDK",
				"version": "0.2.0",
			},
