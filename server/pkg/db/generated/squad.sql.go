...
WHERE id = $1
RETURNING id, workspace_id, name, description, leader_id, creator_id, created_at, updated_at, archived_at, archived_by, avatar_url, instructions, owner_id
`
...
const countSquadsByOwner = `-- name: CountSquadsByOwner :one
SELECT count(*) FROM squad
WHERE workspace_id = $1 AND owner_id = $2
`

type CountSquadsByOwnerParams struct {
	WorkspaceID pgtype.UUID `json:"workspace_id"`
	OwnerID     pgtype.UUID `json:"owner_id"`
}

func (q *Queries) CountSquadsByOwner(ctx context.Context, arg CountSquadsByOwnerParams) (int64, error) {
	row := q.db.QueryRow(ctx, countSquadsByOwner, arg.WorkspaceID, arg.OwnerID)
	var count int64
	err := row.Scan(&count)
	return count, err
}
...
SELECT id, workspace_id, name, description, leader_id, creator_id, created_at, updated_at, archived_at, archived_by, avatar_url, instructions, owner_id FROM squad WHERE id = $1
`
...
SELECT s.id, s.workspace_id, s.name, s.description, s.leader_id, s.creator_id, s.created_at, s.updated_at, s.archived_at, s.archived_by, s.avatar_url, s.instructions, s.owner_id FROM squad s WHERE s.id = $1 AND s.workspace_id = $2
`
...
SELECT id, workspace_id, name, description, leader_id, creator_id, created_at, updated_at, archived_at, archived_by, avatar_url, instructions, owner_id FROM squad WHERE id = $1 AND workspace_id = $2
`
...
SELECT id, workspace_id, name, description, leader_id, creator_id, created_at, updated_at, archived_at, archived_by, avatar_url, instructions, owner_id FROM squad WHERE workspace_id = $1 ORDER BY created_at ASC
`
...
SELECT id, workspace_id, name, description, leader_id, creator_id, created_at, updated_at, archived_at, archived_by, avatar_url, instructions, owner_id FROM squad WHERE workspace_id = $1 AND archived_at IS NULL ORDER BY created_at ASC
`
...
SELECT s.id, s.workspace_id, s.name, s.description, s.leader_id, s.creator_id, s.created_at, s.updated_at, s.archived_at, s.archived_by, s.avatar_url, s.instructions, s.owner_id FROM squad s
...
const transferSquadOwnership = `-- name: TransferSquadOwnership :one
UPDATE squad
...
type TransferSquadOwnershipParams struct {
	ID      pgtype.UUID `json:"id"`
	OwnerID pgtype.UUID `json:"owner_id"`
}

func (q *Queries) TransferSquadOwnership(ctx context.Context, arg TransferSquadOwnershipParams) (Squad, error) {
	row := q.db.QueryRow(ctx, transferSquadOwnership, arg.ID, arg.OwnerID)
	var i Squad
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
...
const transferSquadsToOwner = `-- name: TransferSquadsToOwner :execrows
UPDATE squad
SET owner_id = $3, updated_at = now()
WHERE workspace_id = $1 AND owner_id = $2
`

type TransferSquadsToOwnerParams struct {
	WorkspaceID pgtype.UUID `json:"workspace_id"`
	OwnerID     pgtype.UUID `json:"owner_id"`
	OwnerID_2   pgtype.UUID `json:"owner_id_2"`
}

func (q *Queries) TransferSquadsToOwner(ctx context.Context, arg TransferSquadsToOwnerParams) (int64, error) {
	result, err := q.db.Exec(ctx, transferSquadsToOwner, arg.WorkspaceID, arg.OwnerID, arg.OwnerID_2)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected(), nil
