...
func TestPrintDiskUsageOtherRootsHintSuggestsProfilesWithTasks(t *testing.T) {
	...
	printDiskUsageOtherRootsHint(&out, daemon.DiskUsageReport{
		WorkspacesRoot: filepath.Join(home, "omat_workspaces"),
	}, "", "")
	...
	if !strings.Contains(got, "omat --profile two-tasks daemon disk-usage") {
		t.Fatalf("hint output = %q, want two-tasks profile command", got)
	}
	if !strings.Contains(got, "omat --profile one-task daemon disk-usage") {
		t.Fatalf("hint output = %q, want one-task profile command", got)
	}
	if !strings.Contains(got, "omat --profile 'space profile' daemon disk-usage") {
		t.Fatalf("hint output = %q, want shell-quoted profile command", got)
	}
	if !strings.Contains(got, "omat daemon disk-usage --all-profiles") {
...
func TestPrintDiskUsageOtherRootsHintFiresWhenCurrentRootNonEmpty(t *testing.T) {
	...
	got := out.String()
	if !strings.Contains(got, "omat --profile desktop-host daemon disk-usage") {
...
func TestPrintDiskUsageOtherRootsHintSuggestsDefaultFromNamedProfile(t *testing.T) {
	...
	var out bytes.Buffer
	printDiskUsageOtherRootsHint(&out, daemon.DiskUsageReport{
		WorkspacesRoot: filepath.Join(home, "omat_workspaces_named"),
	}, "named", "")

	got := out.String()
	if !strings.Contains(got, "omat daemon disk-usage  #") {
...
func TestEnumerateDiskUsageRoots(t *testing.T) {
	...
	// Two profiles configured under ~/.ohmyagentteam/profiles, but only one has its
	...
	if roots[0].Profile != "" || roots[0].Root != filepath.Join(home, "omat_workspaces") {
		t.Fatalf("roots[0] = %+v, want default root first", roots[0])
	}
	if roots[1].Profile != "desktop-host" || roots[1].Root != filepath.Join(home, "omat_workspaces_desktop-host") {
...
func TestPrintAggregateDiskUsageShowsRootsAndGrandTotal(t *testing.T) {
	agg := daemon.AggregateDiskUsageReport{
		Roots: []daemon.RootDiskUsage{
			{Profile: "", Report: daemon.DiskUsageReport{
				WorkspacesRoot: "/home/u/omat_workspaces",
			...
			{Profile: "desktop-host", Report: daemon.DiskUsageReport{
				WorkspacesRoot: "/home/u/omat_workspaces_desktop-host",
	...
	if !strings.Contains(got, "/home/u/omat_workspaces_desktop-host") {
...
func mkdirProfile(t *testing.T, home, profile string) {
	t.Helper()
	if err := os.MkdirAll(filepath.Join(home, ".ohmyagentteam", "profiles", profile), 0o755); err != nil {
		t.Fatal(err)
	}
}
...
func writeDiskUsageTaskFile(t *testing.T, home, profile, workspaceID, taskID, rel string) {
	t.Helper()
	path := filepath.Join(home, "omat_workspaces_"+profile, workspaceID, taskID, rel)
	writeDiskUsageFile(t, path)
}

func writeDefaultDiskUsageTaskFile(t *testing.T, home, workspaceID, taskID, rel string) {
	t.Helper()
	path := filepath.Join(home, "omat_workspaces", workspaceID, taskID, rel)
	writeDiskUsageFile(t, path)
}
