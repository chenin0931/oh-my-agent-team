...
// honor OMAT_SERVER_URL when --server-url is not passed, instead of always
...
func TestResolveSelfHostServerURL(t *testing.T) {
	...
	t.Run("env var honored when flag absent", func(t *testing.T) {
		t.Setenv("OMAT_SERVER_URL", "https://api.internal.co")
	...
	t.Run("flag wins over env", func(t *testing.T) {
		t.Setenv("OMAT_SERVER_URL", "https://env.example")
		cmd := newCmd()
	...
	t.Run("falls back to existing config server_url when no flag, env, or --port", func(t *testing.T) {
		t.Setenv("OMAT_SERVER_URL", "")
		existing := cli.CLIConfig{ServerURL: "https://ohmyagentteam-prod-alb.marmot-cloud.com"}
		serverURL, userProvided := resolveSelfHostServerURL(newCmd(), existing)
		if serverURL != "https://ohmyagentteam-prod-alb.marmot-cloud.com" {
	...
	// OMAT_SERVER_URL is documented as a ws:// daemon address; the probe and
	...
	t.Run("normalizes the documented ws:// daemon form", func(t *testing.T) {
		t.Setenv("OMAT_SERVER_URL", "wss://api.internal.co/ws")
		serverURL, userProvided := resolveSelfHostServerURL(newCmd(), cli.CLIConfig{})
		if serverURL != "https://api.internal.co" {
			t.Fatalf("server_url: want normalized https base, got %q", serverURL)
		}
...
// OMAT_APP_URL is honored when the flag is absent.
func TestSelfHostAppURLHonorsEnv(t *testing.T) {
	cmd := &cobra.Command{}
	cmd.Flags().String("app-url", "", "")

	t.Run("env honored when flag absent", func(t *testing.T) {
		t.Setenv("OMAT_APP_URL", "https://app.internal.co")
		if got := cli.FlagOrEnv(cmd, "app-url", "OMAT_APP_URL", ""); got != "https://app.internal.co" {
			t.Fatalf("app_url: want env value, got %q", got)
		}
	})

	t.Run("flag wins over env", func(t *testing.T) {
		t.Setenv("OMAT_APP_URL", "https://env.example")
		if err := cmd.Flags().Set("app-url", "https://flag.example"); err != nil {
			t.Fatalf("set flag: %v", err)
		}
		if got := cli.FlagOrEnv(cmd, "app-url", "OMAT_APP_URL", ""); got != "https://flag.example" {
...
func TestResolveSelfHostAppURL(t *testing.T) {
	...
	t.Run("falls back to existing config when no flag, env, or --frontend-port", func(t *testing.T) {
		...
		existing := cli.CLIConfig{AppURL: "https://ohmyagentteam-console-fc.marmot-cloud.com"}
		if got := resolveSelfHostAppURL(newCmd(), existing); got != "https://ohmyagentteam-console-fc.marmot-cloud.com" {
