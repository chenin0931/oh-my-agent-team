...
import (
	...
	"github.com/chenin0931/oh-my-agent-team/server/internal/daemon"
	"github.com/chenin0931/oh-my-agent-team/server/internal/daemon/execenv"
	logger_pkg "github.com/chenin0931/oh-my-agent-team/server/internal/logger"
...
var daemonDiskUsageCmd = &cobra.Command{
	...
	Long: "Walks the daemon's workspaces root and reports per-task or per-workspace disk usage.\n" +
		...
		"root — the default root plus each ~/.ohmyagentteam/profiles/* root, including the Desktop app's dedicated\n" +
		...
		"overridable via OMAT_GC_ARTIFACT_PATTERNS) so the report stays in sync with what the GC reclaims.\n" +
...
func init() {
	...
	f.String("daemon-id", "", "Unique daemon identifier (env: OMAT_DAEMON_ID)")
	f.String("device-name", "", "Human-readable device name (env: OMAT_DAEMON_DEVICE_NAME)")
	f.String("runtime-name", "", "Runtime display name (env: OMAT_AGENT_RUNTIME_NAME)")
	f.Duration("poll-interval", 0, "Task poll interval (env: OMAT_DAEMON_POLL_INTERVAL)")
	f.Duration("heartbeat-interval", 0, "Heartbeat interval (env: OMAT_DAEMON_HEARTBEAT_INTERVAL)")
	f.Duration("agent-timeout", 0, "Absolute per-task wall-clock cap; 0 = no cap, rely on the watchdogs (env: OMAT_AGENT_TIMEOUT)")
	f.Duration("codex-semantic-inactivity-timeout", 0, "Codex semantic inactivity timeout (env: OMAT_CODEX_SEMANTIC_INACTIVITY_TIMEOUT)")
	f.Int("max-concurrent-tasks", 0, "Max tasks running in parallel (env: OMAT_DAEMON_MAX_CONCURRENT_TASKS)")
	f.Bool("no-auto-update", false, "Disable periodic CLI self-update (env: OMAT_DAEMON_AUTO_UPDATE=false)")
	f.Duration("auto-update-interval", 0, "How often to poll GitHub for a newer release (env: OMAT_DAEMON_AUTO_UPDATE_INTERVAL)")
	...
	rf.String("daemon-id", "", "Unique daemon identifier (env: OMAT_DAEMON_ID)")
	rf.String("device-name", "", "Human-readable device name (env: OMAT_DAEMON_DEVICE_NAME)")
	rf.String("runtime-name", "", "Runtime display name (env: OMAT_AGENT_RUNTIME_NAME)")
	rf.Duration("poll-interval", 0, "Task poll interval (env: OMAT_DAEMON_POLL_INTERVAL)")
	rf.Duration("heartbeat-interval", 0, "Heartbeat interval (env: OMAT_DAEMON_HEARTBEAT_INTERVAL)")
	rf.Duration("agent-timeout", 0, "Absolute per-task wall-clock cap; 0 = no cap, rely on the watchdogs (env: OMAT_AGENT_TIMEOUT)")
	rf.Duration("codex-semantic-inactivity-timeout", 0, "Codex semantic inactivity timeout (env: OMAT_CODEX_SEMANTIC_INACTIVITY_TIMEOUT)")
	rf.Int("max-concurrent-tasks", 0, "Max tasks running in parallel (env: OMAT_DAEMON_MAX_CONCURRENT_TASKS)")
	rf.Bool("no-auto-update", false, "Disable periodic CLI self-update (env: OMAT_DAEMON_AUTO_UPDATE=false)")
	rf.Duration("auto-update-interval", 0, "How often to poll GitHub for a newer release (env: OMAT_DAEMON_AUTO_UPDATE_INTERVAL)")
	...
	df.Bool("all-profiles", false, "Scan every workspace root (default root + all ~/.ohmyagentteam/profiles/* roots, incl. the Desktop app's) and report a combined total")
...
// Empty profile → ~/.ohmyagentteam/, named profile → ~/.ohmyagentteam/profiles/<name>/.
...
func buildDaemonStartArgs(cmd *cobra.Command) []string {
	...
	// (= no cap), so it can override an environment OMAT_AGENT_TIMEOUT.
...
func runDaemonForeground(cmd *cobra.Command) error {
	...
	serverURL := cli.FlagOrEnv(cmd, "server-url", "OMAT_SERVER_URL", "")
	if serverURL == "" {
	...
	cfg.LaunchedBy = os.Getenv("OMAT_LAUNCHED_BY")
...
// each ~/.ohmyagentteam/profiles/* root) and renders a per-root breakdown with a
...
// orientation even when empty), then each ~/.ohmyagentteam/profiles/* root that
// exists on disk, sorted by profile name. Roots that resolve to the same path
// (e.g. when OMAT_WORKSPACES_ROOT pins every profile to one directory) are
...
func printDiskUsageOtherRootsHint(w io.Writer, report daemon.DiskUsageReport, profile, rootOverride string) {
	...
	fmt.Fprintln(w, "Run 'omat daemon disk-usage --all-profiles' for a combined total across all roots.")
}
...
func diskUsageProfileSuggestions(currentProfile, currentRoot string) []diskUsageProfileSuggestion {
		...
		if root, err := daemon.ResolveWorkspacesRoot("", ""); err == nil && !samePath(root, currentRoot) {
			if taskCount := countDiskUsageTaskDirs(root); taskCount > 0 {
				out = append(out, diskUsageProfileSuggestion{
					Profile:   "",
					Command:   "omat daemon disk-usage",
					Root:      root,
					TaskCount: taskCount,
				})
			}
	...
	for _, entry := range entries {
		...
		out = append(out, diskUsageProfileSuggestion{
			Profile:   profile,
			Command:   "omat --profile " + shellQuoteArg(profile) + " daemon disk-usage",
			Root:      root,
			TaskCount: taskCount,
		})
	}
...
func profilesRootDir() (string, error) {
	...
	return filepath.Join(home, ".ohmyagentteam", "profiles"), nil
}
