	...
	if _, err := normalizeUUIDPrefix(trimmed); err == nil {
		return resolvedID{}, fmt.Errorf(
			"issue ref %q looks like a short UUID prefix; short prefixes are no longer supported for issues. "+
				"Use the issue key (e.g. PROJ-123) shown by `omat issue list`, or pass the full UUID (run a list command with --full-id to copy it)",
			input,
		)
	}
	return resolvedID{}, fmt.Errorf(
		"issue ref %q is not a recognized issue reference; use the issue key (e.g. PROJ-123) shown by `omat issue list`, or pass the full UUID",
		input,
	)
}
...
func resolveEpicRef(ctx context.Context, client *cli.APIClient, input string) (resolvedID, error) {
	trimmed := strings.TrimSpace(input)
	if trimmed == "" {
		return resolvedID{}, fmt.Errorf("epic id is required")
	}
	if !looksLikeIssueIdentifier(trimmed) && !uuidRegexp.MatchString(trimmed) {
		return resolvedID{}, fmt.Errorf("epic ref %q must be an Epic key or full UUID", input)
	}
	var epic map[string]any
	if err := client.GetJSON(ctx, "/api/epics/"+url.PathEscape(trimmed), &epic); err != nil {
		return resolvedID{}, err
	}
	display := strVal(epic, "identifier")
	if display == "" {
		display = strVal(epic, "id")
	}
	return resolvedID{ID: strVal(epic, "id"), Display: display}, nil
}
...
func resolveTaskRunID(ctx context.Context, client *cli.APIClient, issueID, input string) (resolvedID, error) {
	...
	if strings.TrimSpace(issueID) == "" {
		return resolvedID{}, fmt.Errorf("short task run prefixes require --issue <issue-id>; pass a full task UUID or run `omat issue runs <issue-id> --full-id`")
	}
