...
func TestResolveAppURL(t *testing.T) {
	...
	t.Run("prefers OMAT_APP_URL", func(t *testing.T) {
		t.Setenv("OMAT_APP_URL", "http://localhost:14000")
...
func TestBrowserLoginInstructionsSSHRemoteHint(t *testing.T) {
	const loginURL = "https://ohmyagentteam.com/login?cli_callback=http%3A%2F%2Flocalhost%3A43689%2Fcallback"
...
func TestLoginTokenFlagWiring(t *testing.T) {
	...
	if tokenFlag.NoOptDefVal != tokenPromptSentinel {
		t.Fatalf("loginCmd --token NoOptDefVal = %q, want %q (legacy `omat login --token` prompt mode would break)", tokenFlag.NoOptDefVal, tokenPromptSentinel)
	}
}
...
// `omat login --token`. The original implementation hardcoded `mul_`
// only, which rejected legitimate OhMyAgentTeam Cloud Node PATs (`mcn_`) at
