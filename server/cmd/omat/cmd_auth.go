...
// loginTokenPrefixes are the token prefixes `omat login --token` accepts.
// The CLI used to hardcode `mul_` only, which made it impossible to log in
// with a OhMyAgentTeam Cloud Node PAT (`mcn_`) even though the server happily
...
var authCmd = &cobra.Command{
	Use:   "auth",
	Short: "Authenticate the OhMyAgentTeam CLI",
}
...
func resolveToken(cmd *cobra.Command) string {
	if v := strings.TrimSpace(os.Getenv("OMAT_TOKEN")); v != "" {
		return v
	}
	...
	// inDaemonManagedExecutionContext already covers the OMAT_DAEMON_PORT
	// signal for subprocesses that lost OMAT_AGENT_ID / OMAT_TASK_ID.
...
func resolveAppURL(cmd *cobra.Command) string {
	for _, key := range []string{"OMAT_APP_URL", "FRONTEND_ORIGIN"} {
	...
	fmt.Fprintln(os.Stderr, "No app URL configured. Run 'omat setup' first.")
	os.Exit(1)
	return "" // unreachable
}
...
func runAuthLoginBrowser(cmd *cobra.Command) error {
	...
	if err != nil {
		return cli.WithUserMessage("Sign-in did not complete: the server could not issue an access token for the CLI. Run `omat login` again.", err)
	}
	...
	if err := patClient.GetJSON(ctx, "/api/me", &me); err != nil {
		return cli.WithUserMessage("Sign-in did not complete: the server did not accept the new credential. Run `omat login` again.", err)
	}
...
func runAuthLoginToken(cmd *cobra.Command, providedToken string) error {
	...
	if err := client.GetJSON(ctx, "/api/me", &me); err != nil {
		return cli.WithUserMessage("Could not sign in with that token — make sure it is valid and not expired, then run `omat login --token <token>` again.", err)
	}
...
func runAuthStatus(cmd *cobra.Command, _ []string) error {
	...
	if token == "" {
		fmt.Fprintln(os.Stderr, "Not authenticated. Run 'omat login' to authenticate.")
		return nil
	}
...
<title>OhMyAgentTeam — Authenticated</title>
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .brand { display: flex; align-items: center; justify-content: center; gap: 9px; margin-bottom: 8px; font-family: Georgia, serif; font-size: 17px; font-weight: 600; }
  .brand-mark { position: relative; display: inline-block; width: 22px; height: 18px; border: 2px solid var(--fg); border-radius: 50%; }
  .brand-mark::before { content: ""; position: absolute; width: 6px; height: 6px; border-radius: 50%; background: #ff6b5f; top: -4px; right: -3px; box-shadow: -2px 14px 0 #f0b429; }
  .brand-mark::after { content: ""; position: absolute; width: 6px; height: 6px; background: #43b97f; top: 2px; right: -7px; transform: rotate(18deg); }
...
<body>
  <div class="card">
    ...
    <div class="brand"><span class="brand-mark"></span><span>OhMyAgentTeam</span></div>
