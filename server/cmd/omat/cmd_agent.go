...
// custom_env values — `omat agent list / get / update` no longer
...
func init() {
	...
	// `omat agent env set <id>` — that path is owner/admin-only,
...
func newAPIClient(cmd *cobra.Command) (*cli.APIClient, error) {
	...
	if serverURL == "" {
		return nil, fmt.Errorf("server URL not set: use --server-url flag, OMAT_SERVER_URL env, or 'omat config set server_url <url>'")
	}
	if inDaemonManagedExecutionContext() && !strings.HasPrefix(token, "mat_") {
		// When the ONLY daemon signal is a workdir marker (no OMAT_AGENT_ID /
		// OMAT_TASK_ID / OMAT_DAEMON_PORT), the likeliest cause outside a
		...
		if !inAgentExecutionContext() && os.Getenv("OMAT_DAEMON_PORT") == "" {
			if markerPath := daemonTaskContextMarkerPath(); markerPath != "" {
				return nil, fmt.Errorf("agent execution context requires OMAT_TOKEN to be a task-scoped mat_ token; detected a daemon task marker at %s — if you are not running inside an agent task this is likely a leftover, remove it and retry", markerPath)
			}
		}
		return nil, fmt.Errorf("agent execution context requires OMAT_TOKEN to be a task-scoped mat_ token")
	}

	client := cli.NewAPIClient(serverURL, workspaceID, token)
	// When running inside a daemon task, attribute actions to the agent.
	if agentID := os.Getenv("OMAT_AGENT_ID"); agentID != "" {
		client.AgentID = agentID
	}
	if taskID := os.Getenv("OMAT_TASK_ID"); taskID != "" {
...
func resolveServerURL(cmd *cobra.Command) string {
	val := cli.FlagOrEnv(cmd, "server-url", "OMAT_SERVER_URL", "")
	if val != "" {
	...
	fmt.Fprintln(os.Stderr, "No server configured. Run 'omat setup' first.")
	os.Exit(1)
	return "" // unreachable
}
...
func inAgentExecutionContext() bool {
	return os.Getenv("OMAT_AGENT_ID") != "" || os.Getenv("OMAT_TASK_ID") != ""
}

// inDaemonManagedExecutionContext reports whether the CLI is being invoked
// from inside a daemon-managed agent task. OMAT_DAEMON_PORT is included as
// a defense-in-depth marker for subprocesses that lose OMAT_AGENT_ID or
// OMAT_TASK_ID but still run under the daemon environment. In this context
// workspace and token must come from daemon-provided env; falling back to
// user-global ~/.ohmyagentteam/config.json can make agent writes land as a member.
func inDaemonManagedExecutionContext() bool {
	return inAgentExecutionContext() || os.Getenv("OMAT_DAEMON_PORT") != "" || hasDaemonTaskContextMarker()
}
...
func resolveWorkspaceID(cmd *cobra.Command) string {
	val := cli.FlagOrEnv(cmd, "workspace-id", "OMAT_WORKSPACE_ID", "")
	if val != "" {
		return val
	}
...
func requireWorkspaceID(cmd *cobra.Command) (string, error) {
	...
	if id == "" {
		if inDaemonManagedExecutionContext() {
			return "", fmt.Errorf("workspace_id is required: OMAT_WORKSPACE_ID must be set by the daemon in agent execution context (no fallback to user config)")
		}
		return "", fmt.Errorf("workspace_id is required: use --workspace-id flag, set OMAT_WORKSPACE_ID env, or run 'omat config set workspace_id <id>'")
	}
	return id, nil
}
...
func runAgentUpdate(cmd *cobra.Command, args []string) error {
	...
	if len(body) == 0 {
		return fmt.Errorf("no fields to update; use --name, --description, --instructions, --runtime-id, --runtime-config, --model, --thinking-level, --custom-args, --mcp-config, --visibility, --status, or --max-concurrent-tasks (env vars now live behind `omat agent env set <id>`)")
	}
