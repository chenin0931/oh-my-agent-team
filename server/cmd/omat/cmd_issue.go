...
var issueReorderCmd = &cobra.Command{
	...
	Long: "Reposition an issue inside its current status column by computing a new\n" +
		...
		"different column, change its status first with `omat issue status`.",
	Args: exactArgs(1),
...
func init() {
	...
	issueListCmd.Flags().String("type", "", "Filter by executable work item type: issue or subtask")
	issueListCmd.Flags().String("epic-id", "", "Filter by Epic UUID")
	...
	issueCreateCmd.Flags().String("type", "", "Work item type: issue or subtask")
	issueCreateCmd.Flags().String("epic-id", "", "Epic container ID for a normal issue")
	...
	issueCreateCmd.Flags().String("acceptance-criteria", "", "Acceptance criteria in Markdown")
	issueCreateCmd.Flags().Bool("acceptance-criteria-stdin", false, "Read acceptance criteria from stdin")
	issueCreateCmd.Flags().String("acceptance-criteria-file", "", "Read acceptance criteria from a UTF-8 file")
	...
	issueCommentListCmd.Flags().String("before", "", "Cursor (RFC3339Nano timestamp). With --recent: thread cursor (last_activity_at). With --thread + --tail: reply cursor (reply created_at). Read from the X-OhMyAgentTeam-Next-Before response header; must be paired with --before-id.")
	issueCommentListCmd.Flags().String("before-id", "", "Cursor UUID. With --recent: thread root UUID. With --thread + --tail: oldest reply UUID. Read from the X-OhMyAgentTeam-Next-Before-Id response header; must be paired with --before.")
...
func runIssueList(cmd *cobra.Command, _ []string) error {
	...
	if v, _ := cmd.Flags().GetString("type"); v != "" {
		if v != "issue" && v != "subtask" {
			return fmt.Errorf("--type must be issue or subtask; use `omat epic` for planning containers")
		}
		params.Set("issue_type", v)
	}
	if v, _ := cmd.Flags().GetString("epic-id"); v != "" {
		epic, err := resolveEpicRef(ctx, client, v)
		if err != nil {
			return fmt.Errorf("resolve epic: %w", err)
		}
		params.Set("epic_id", epic.ID)
	}
...
func quickCreateAttachmentIDsFromEnv() ([]string, error) {
	raw := strings.TrimSpace(os.Getenv("OMAT_QUICK_CREATE_ATTACHMENT_IDS"))
	if raw == "" {
		return nil, nil
	}
	var ids []string
	if err := json.Unmarshal([]byte(raw), &ids); err != nil {
		return nil, fmt.Errorf("parse OMAT_QUICK_CREATE_ATTACHMENT_IDS: %w", err)
	}
...
func runIssueCreate(cmd *cobra.Command, _ []string) error {
	...
	quickCreateMode := strings.TrimSpace(os.Getenv("OMAT_QUICK_CREATE_MODE"))
	quickCreateDefaultStatus := strings.TrimSpace(os.Getenv("OMAT_QUICK_CREATE_DEFAULT_STATUS"))
	if quickCreateMode == "planning" {
		if statusFlag == "" {
			if quickCreateDefaultStatus == "" {
				quickCreateDefaultStatus = "backlog"
			}
			if quickCreateDefaultStatus != "backlog" {
				return fmt.Errorf("planning quick-create default status must be backlog, got %q", quickCreateDefaultStatus)
			}
			statusFlag = quickCreateDefaultStatus
		} else if statusFlag != "backlog" {
			return fmt.Errorf("planning quick-create requires --status backlog, got %q", statusFlag)
		}
	}
	if statusFlag != "" {
	...
	if issueType != "" && issueType != "issue" && issueType != "subtask" {
		return fmt.Errorf("--type must be issue or subtask; use `omat epic create` for planning containers")
	}
	...
	criteria, hasCriteria, err := resolveTextFlag(cmd, "acceptance-criteria")
	if err != nil {
		return err
	}
	if hasCriteria {
	...
	// Quick-create stamp: when the daemon sets OMAT_QUICK_CREATE_TASK_ID
	// before invoking the agent, the agent's `omat issue create` call
...
func reorderTargetNotInColumnError(ctx context.Context, client *cli.APIClient, otherRef, issueRef resolvedID, status string) error {
	if other, err := fetchIssue(ctx, client, otherRef.ID); err == nil {
		if otherStatus := strVal(other, "status"); otherStatus != "" && otherStatus != status {
			return fmt.Errorf("issue %s is in the %q column but %s is in %q; move one with `omat issue status` first, or pick a target in the same column", otherRef.Display, otherStatus, issueRef.Display, status)
		}
	}
...
func runIssueCommentList(cmd *cobra.Command, args []string) error {
	...
	if nb := respHeaders.Get("X-OhMyAgentTeam-Next-Before"); nb != "" {
		if nbid := respHeaders.Get("X-OhMyAgentTeam-Next-Before-Id"); nbid != "" {
...
func resolveAssignee(ctx context.Context, client *cli.APIClient, name string, kinds assigneeKinds) (string, string, error) {
	if client.WorkspaceID == "" {
		return "", "", fmt.Errorf("workspace ID is required to resolve assignees; use --workspace-id or set OMAT_WORKSPACE_ID")
	}
...
// from `omat workspace member list --output json`) should use this instead of
...
func resolveAssigneeByID(ctx context.Context, client *cli.APIClient, id string, kinds assigneeKinds) (string, string, error) {
	...
	for _, m := range members {
		if strings.EqualFold(strVal(m, "user_id"), input) || strings.EqualFold(strVal(m, "id"), input) {
