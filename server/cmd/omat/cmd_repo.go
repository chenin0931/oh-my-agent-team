...
func runRepoCheckout(cmd *cobra.Command, args []string) error {
	...
	daemonPort := os.Getenv("OMAT_DAEMON_PORT")
	if daemonPort == "" {
		return fmt.Errorf("OMAT_DAEMON_PORT not set (this command is intended to be run by an agent inside a daemon task)")
	}

	workspaceID := os.Getenv("OMAT_WORKSPACE_ID")
	agentName := os.Getenv("OMAT_AGENT_NAME")
	taskID := os.Getenv("OMAT_TASK_ID")
