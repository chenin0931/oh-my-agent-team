...
func TestRunIssueCreateSendsHierarchyAndAcceptanceCriteria(t *testing.T) {
	var body map[string]any
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Method == http.MethodGet && r.URL.Path == "/api/epics/epic-1" {
			json.NewEncoder(w).Encode(map[string]any{"id": "epic-uuid", "identifier": "MUL-EPIC", "title": "Launch epic"})
			return
		}
		...
		json.NewEncoder(w).Encode(map[string]any{
			"id": "issue-1", "identifier": "MUL-1", "title": "Child plan", "status": "backlog", "priority": "none",
		})
	}))
	defer srv.Close()

	t.Setenv("OMAT_SERVER_URL", srv.URL)
	t.Setenv("OMAT_WORKSPACE_ID", "ws-1")
	t.Setenv("OMAT_TOKEN", "test-token")
	cmd := newIssueCreateTestCmd()
	_ = cmd.Flags().Set("title", "Child plan")
	_ = cmd.Flags().Set("type", "issue")
	_ = cmd.Flags().Set("epic-id", "epic-1")
	_ = cmd.Flags().Set("acceptance-criteria", `Approved\nand scheduled`)
	if err := runIssueCreate(cmd, nil); err != nil {
		t.Fatalf("runIssueCreate: %v", err)
	}
	if body["issue_type"] != "issue" || body["epic_id"] != "epic-uuid" {
		t.Fatalf("hierarchy body = %#v", body)
	}
	if body["acceptance_criteria"] != "Approved\nand scheduled" {
		t.Fatalf("acceptance_criteria = %#v", body["acceptance_criteria"])
	}
}
...
func TestRunIssueCreateSendsExistingAttachmentIDs(t *testing.T) {
	...
	t.Setenv("OMAT_WORKSPACE_ID", "ws-1")
	t.Setenv("OMAT_TOKEN", "test-token")
	t.Setenv("OMAT_QUICK_CREATE_ATTACHMENT_IDS", `["att-env","att-shared"]`)
...
func TestRunIssueCreateOmitsStatusByDefault(t *testing.T) {
	var body map[string]any
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/api/issues" {
			http.NotFound(w, r)
	...
	cmd := newIssueCreateTestCmd()
	_ = cmd.Flags().Set("title", "Default parked work")
	if err := runIssueCreate(cmd, nil); err != nil {
		t.Fatalf("runIssueCreate: %v", err)
	}
	if _, ok := body["status"]; ok {
		t.Fatalf("status should be omitted by default so the server applies backlog, got %#v", body["status"])
	}
}

func TestRunIssueCreatePlanningQuickCreateDefaultsStatusBacklog(t *testing.T) {
	var body map[string]any
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/api/issues" {
			http.NotFound(w, r)
	...
	t.Setenv("OMAT_WORKSPACE_ID", "ws-1")
	t.Setenv("OMAT_TOKEN", "test-token")
	t.Setenv("OMAT_QUICK_CREATE_MODE", "planning")
	t.Setenv("OMAT_QUICK_CREATE_DEFAULT_STATUS", "backlog")

	cmd := newIssueCreateTestCmd()
	_ = cmd.Flags().Set("title", "Planned work")
	if err := runIssueCreate(cmd, nil); err != nil {
		t.Fatalf("runIssueCreate: %v", err)
	}
	if got := body["status"]; got != "backlog" {
		t.Fatalf("status = %#v, want backlog", got)
	}
}

func TestRunIssueCreatePlanningQuickCreateRejectsActiveStatus(t *testing.T) {
	called := false
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		called = true
	...
	cmd := newIssueCreateTestCmd()
	_ = cmd.Flags().Set("title", "Should not create")
	_ = cmd.Flags().Set("status", "todo")
	if err := runIssueCreate(cmd, nil); err == nil {
		t.Fatalf("expected planning quick-create status error")
	}
	if called {
		t.Fatalf("planning quick-create with todo status must not send create request")
	}
}
...
func TestRunIssuePullRequestsListsLinkedPRsAsJSON(t *testing.T) {
		...
		case "/api/issues/issue-uuid/pull-requests":
			json.NewEncoder(w).Encode(map[string]any{
				"pull_requests": []map[string]any{
					{
						"url":    "https://github.com/chenin0931/oh-my-agent-team/pull/42",
						"number": float64(42),
						"state":  "open",
	...
	if pr["url"] != "https://github.com/chenin0931/oh-my-agent-team/pull/42" || pr["number"] != float64(42) || pr["state"] != "open" || pr["title"] != "MUL-2818 add issue PR CLI" {
...
func TestRunIssuePullRequestsTableIncludesCoreFields(t *testing.T) {
	...
	out, _ := io.ReadAll(r)
	text := string(out)
	for _, want := range []string{"NUMBER", "STATE", "TITLE", "URL", "42", "open", "MUL-2818 add issue PR CLI", "https://github.com/chenin0931/oh-my-agent-team/pull/42"} {
...
func TestResolveIssueRef(t *testing.T) {
	...
	t.Run("short UUID prefix is rejected without any HTTP call", func(t *testing.T) {
		...
		if msg := err.Error(); !strings.Contains(msg, "PROJ-") {
...
// chenin0931/oh-my-agent-team#1620: when one name is a substring of another (e.g.
...
// chenin0931/oh-my-agent-team#1620: passing a full UUID or its 8-char prefix must
// resolve directly without going through name matching.
func TestResolveAssigneeByID(t *testing.T) {
	membersResp := []map[string]any{
		{"id": "eeeeeeee-1111-1111-1111-111111111111", "user_id": "aaaaaaaa-1111-1111-1111-111111111111", "name": "Alice"},
	}
	agentsResp := []map[string]any{
		{"id": "f656eab8-1111-1111-1111-111111111111", "name": "reviewer"},
		{"id": "9b0ff9a2-2222-2222-2222-222222222222", "name": "peer-reviewer"},
...
func TestResolveAssigneeByIDStrict(t *testing.T) {
	...
	t.Run("member row UUID resolves to the member user id", func(t *testing.T) {
		aType, aID, err := resolveAssigneeByID(ctx, client, "eeeeeeee-1111-1111-1111-111111111111", issueAssigneeKinds)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if aType != "member" || aID != "aaaaaaaa-1111-1111-1111-111111111111" {
			t.Errorf("got (%q, %q), want Alice user id", aType, aID)
		}
	})
...
// `omat issue subscriber add --user "<SquadName>"` or
// `omat project create --lead "<SquadName>"` would resolve to
...
func TestRunIssueCommentList_ThreadTailPassesThroughAndPrintsReplyCursor(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Method == http.MethodGet && strings.HasSuffix(r.URL.Path, "/comments") {
			...
			w.Header().Set("X-OhMyAgentTeam-Next-Before", "2026-01-01T00:00:00.000000001Z")
			w.Header().Set("X-OhMyAgentTeam-Next-Before-Id", "00000000-0000-0000-0000-000000000999")
			w.Write([]byte("[]"))
			return
		}
		t.Errorf("unexpected request: %s %s", r.Method, r.URL.String())
...
func TestRunIssueCommentList_RecentStillLabelsCursorAsThread(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("X-OhMyAgentTeam-Next-Before", "2026-01-01T00:00:00.000000001Z")
		w.Header().Set("X-OhMyAgentTeam-Next-Before-Id", "00000000-0000-0000-0000-000000000777")
		w.Write([]byte("[]"))
	}))
	defer srv.Close()
...
func TestRunIssueListRejectsInvalidSortAndDirection(t *testing.T) {
	t.Setenv("OMAT_SERVER_URL", "http://127.0.0.1:0")
	t.Setenv("OMAT_WORKSPACE_ID", "ws-1")
	t.Setenv("OMAT_TOKEN", "test-token")
