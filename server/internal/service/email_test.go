	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			p := buildInvitationParams(
				"noreply@ohmyagentteam.com",
				"invitee@example.com",
				tt.inviter,
				tt.workspace,
				"https://app.ohmyagentteam.com/invite/abc-123",
			)
...
func TestBuildInvitationParams_SubjectTruncated(t *testing.T) {
	...
	// Template: "Alice invited you to <ws> on OhMyAgentTeam"
	// ws is capped at maxSubjectFieldRunes; overall subject should also be bounded.
	maxExpected := len("Alice invited you to  on OhMyAgentTeam") + maxSubjectFieldRunes
...
func TestBuildInvitationParams_ToAndFromPassedThrough(t *testing.T) {
	...
	if p.From != "noreply@ohmyagentteam.com" {
		t.Errorf("From = %q", p.From)
	}
	if len(p.To) != 1 || p.To[0] != "invitee@example.com" {
		t.Errorf("To = %v", p.To)
	}
	if !strings.Contains(p.Html, "https://app.ohmyagentteam.com/invite/abc") {
