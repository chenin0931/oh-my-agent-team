...
func NewEmailService() *EmailService {
	...
	if from == "" {
		from = "noreply@ohmyagentteam.com"
	}
	...
	default:
		fmt.Println("EmailService: DEV mode — codes printed to stdout (set OMAT_DEV_VERIFICATION_CODE in .env for a fixed local code)")
	}
...
func (s *EmailService) SendVerificationCode(to, code string) error {
	...
	if s.smtpHost != "" {
		return s.sendSMTP(to, "Your OhMyAgentTeam verification code", body)
	}
	if s.client == nil {
	...
	params := &resend.SendEmailRequest{
		From:    s.fromEmail,
		To:      []string{to},
		Subject: "Your OhMyAgentTeam verification code",
		Html:    body,
	}
...
func (s *EmailService) SendInvitationEmail(to, inviterName, workspaceName, invitationID string) error {
	...
	if appURL == "" {
		appURL = "https://app.ohmyagentteam.com"
	}
...
func buildInvitationParams(from, to, inviterName, workspaceName, inviteURL string) *resend.SendEmailRequest {
	...
	return &resend.SendEmailRequest{
		From:    from,
		To:      []string{to},
		Subject: fmt.Sprintf("%s invited you to %s on OhMyAgentTeam", subjectInviter, subjectWorkspace),
		Html: fmt.Sprintf(
			`<div style="font-family: sans-serif; max-width: 480px; margin: 0 auto;">
				<h2>You're invited to join %s</h2>
				<p><strong>%s</strong> invited you to collaborate in the <strong>%s</strong> workspace on OhMyAgentTeam.</p>
