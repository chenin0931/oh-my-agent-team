...
| Pattern: `` `\[@?(.+?)\]\(mention://(member\|agent\|squad\|epic\|issue\|all)/([0-9a-fA-F-]+\|all)\)` `` | `server/internal/util/mention.go` |
| `<type>` group = `member \| agent \| squad \| epic \| issue \| all` | `server/internal/util/mention.go` |
...
| `mention://epic/<uuid>` parses as a side-effect-free Epic reference | `server/internal/util/mention_test.go` |
...
| **`member`, `epic`, and `issue` mentions reach neither branch — they enqueue NOTHING.** Only the `squad` and `agent` branches add triggers. | `server/internal/handler/comment.go` (`resolveMentionedAgentCommentTriggers`) |
...
| --- | --- | --- |
| `workspace member list` | `user_id` (NOT the membership-row id) | `server/cmd/omat/cmd_workspace.go:465` |
| `agent list` | `id` | `server/cmd/omat/cmd_agent.go:365` |
| `squad list` | `id` | `server/cmd/omat/cmd_squad.go:57` |
