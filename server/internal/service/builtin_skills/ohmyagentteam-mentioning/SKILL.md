...
This skill states WHAT a mention link does in the OhMyAgentTeam backend, traced to
...
five entity `<type>` values plus the `all` sentinel, and the `<id>` group
accepts only hex characters and dashes, OR the literal string `all`:

    (member|agent|squad|epic|issue|all)/([0-9a-fA-F-]+|all)
...
- a person → `omat workspace member list --output json` → use `user_id`
- an agent → `omat agent list --output json` → use `id`
- a squad  → `omat squad list --output json` → use `id`
...
## Step 2 — entity types and exactly what each enqueues
...
| reference an Epic    | `epic`   | epic.id         | renders a planning-container link; enqueues NOTHING      |
...
A `member`, `epic`, or `issue` mention reaches neither branch, so it enqueues no task.
...
- **A private agent you cannot access:** skipped — the mention path gates on
  ...
  one; the child-done wake is ungated — see the ohmyagentteam-squads skill).
...
Correct:
  1. `omat workspace member list --output json`  → Alice's `user_id` = 7f3a…
