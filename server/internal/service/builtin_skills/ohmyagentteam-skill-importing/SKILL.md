...
A skill is installed for OhMyAgentTeam only when it exists in the current workspace's
...
```bash
omat skill import --url <url> --output json              # hosted source
omat skill import --file <path-to.skill> --output json   # local archive
```
...
environment, not the OhMyAgentTeam workspace DB, so OhMyAgentTeam cannot manage or bind it.

## Supported URL source families

`detectImportSource` accepts these hosts (and `www.` variants). Pass any of these
forms to `omat skill import --url <url> --output json`:

```bash
omat skill import --url clawhub.ai/owner/skill --output json
omat skill import --url skills.sh/owner/repo/skill --output json
omat skill import --url github.com/owner/repo --output json
omat skill import --url github.com/owner/repo/tree/main/path/to/skill --output json
omat skill import --url github.com/owner/repo/blob/main/path/to/SKILL.md --output json
```
...
`omat skill import --file <path> --output json` imports a skill from a local
...
After the final `omat agent skills list <agent-id> --output json`, verify the
target skill id is present before claiming the skill is available to that agent.

## Additive add vs replace-all set

`omat agent skills add` is additive: the server inserts the assignments without
clearing existing ones (`AddAgentSkills`).

`omat agent skills set` is replace-all: the server clears every current
...
Default behavior is safe: `omat skill import --url <url>` is equivalent to
...
omat skill import --url https://skills.sh/acme/repo/review-helper --output json

# Replace the existing same-name skill, preserving its ID and agent bindings.
omat skill import --url https://skills.sh/acme/repo/review-helper --on-conflict overwrite --output json

# Keep the existing skill and import a copy such as review-helper-2.
omat skill import --url https://skills.sh/acme/repo/review-helper --on-conflict rename --output json

# Batch-friendly behavior: leave the existing skill alone and mark it skipped.
omat skill import --url https://skills.sh/acme/repo/review-helper --on-conflict skip --output json
```
...
```bash
omat skill get <skill-id> --output json
```
...
Incorrect (bypasses OhMyAgentTeam):

```bash
npx skills add https://skills.sh/owner/repo/skill
```

The skill may exist locally, but OhMyAgentTeam cannot manage it as a workspace skill.
...
```bash
omat skill import --url https://skills.sh/owner/repo/skill --output json
```
