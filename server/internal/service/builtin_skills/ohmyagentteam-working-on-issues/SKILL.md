---
name: ohmyagentteam-working-on-issues
description: "Use when working on a OhMyAgentTeam issue after the runtime has provided the trigger context — to apply the product contracts the runtime brief does not encode: how PR linking differs from close intent, how to read a linked PR's real state via the pull-requests CLI, which metadata keys are high-signal, what status changes trigger on the server, and how sub-issue create status (todo vs backlog) controls whether assigned agents start immediately."
user-invocable: false
allowed-tools: Bash(ohmyagentteam *), Bash(git *), Bash(gh *)
---

# Working on OhMyAgentTeam issues
...
For building mention links, load `ohmyagentteam-mentioning` instead — not this skill.
For Project-level outcome planning, load `ohmyagentteam-planning-epics`: an Epic is a
non-executable container and must never be passed to the Issue commands below.
...
This is the link that `omat issue pull-requests` reads back — but see the
...
**excluded from `omat issue pull-requests`** (and the issue's right-side PR
...
is to open or update a PR before posting the final OhMyAgentTeam issue comment, unless
...
When a step depends on PR state, query OhMyAgentTeam's link table — do not infer it
...
```bash
omat issue metadata set <issue-id> --key pr_url --value <url>
omat issue metadata delete <issue-id> --key <stale-key>
```
...
```bash
omat issue create --title "..." --parent <issue-id> --assignee <agent> --status todo
```

Strictly serial children — park later steps, promote one at a time:

```bash
omat issue create --title "Step 2: ..." --parent <issue-id> --assignee <agent> --status backlog
omat issue status <child-id> todo   # promote when the previous step is truly done
```
...
omat issue create --title "Research A" --parent <id> --assignee <agent> --stage 1 --status todo
omat issue create --title "Research B" --parent <id> --assignee <agent> --stage 1 --status todo
omat issue create --title "Build"      --parent <id> --assignee <agent> --stage 2 --status backlog
omat issue create --title "Ship"       --parent <id> --assignee <agent> --stage 3 --status backlog
```
...
```bash
omat issue children <parent-id>             # sub-issues grouped by stage
omat issue status <stage-2-child-id> todo   # promote when its deps are met
```
...
omat issue create --title "Step 2" --parent <issue-id> --assignee <agent> --status todo
omat issue create --title "Step 3" --parent <issue-id> --assignee <agent> --status todo

# correct — stage them; Stage 1 runs, later stages park and are promoted as
# each stage's barrier closes
omat issue create --title "Step 1" --parent <issue-id> --assignee <agent> --stage 1 --status todo
omat issue create --title "Step 2" --parent <issue-id> --assignee <agent> --stage 2 --status backlog
omat issue create --title "Step 3" --parent <issue-id> --assignee <agent> --stage 3 --status backlog
```
