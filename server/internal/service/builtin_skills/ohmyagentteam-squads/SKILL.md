---
name: ohmyagentteam-squads
description: "Use when creating, inspecting, updating, assigning, mentioning, or debugging OhMyAgentTeam squads. Explains what squads are, squad/member fields, CLI commands, leader routing, issue assignment, comments, mentions, autopilot behavior, leader briefing, side effects, and product-gap handling."
user-invocable: false
allowed-tools: Bash(ohmyagentteam *)
---

# OhMyAgentTeam Squads
...
omat squad get <squad-id> --output json
omat squad member list <squad-id> --output json
omat issue comment list <issue-id> --recent 10 --output json
```
...
```bash
omat squad --help
omat squad member --help
omat issue update --help
...
A OhMyAgentTeam squad is a workspace routing and coordination object.
...
omat squad get <squad-id> --output json
omat squad create --name <name> --leader <agent-name-or-id> --output json
omat squad update <squad-id> --instructions "<leader coordination policy>" --output json
omat squad delete <squad-id>
```
...
```bash
omat squad member list <squad-id> --output json
omat squad member add <squad-id> --member-id <id> --type agent|member --role <role> --output json
omat squad member remove <squad-id> --member-id <id> --type agent|member
omat squad member set-role <squad-id> --member-id <id> --member-type agent|member --role <role> --output json
```

Squad leader evaluation command:

```bash
omat squad activity <issue-id> action|no_action|failed --reason "<why>" --output json
```
...
```bash
omat issue get <issue-id> --output json
omat issue update <issue-id> --help
omat issue comment list <issue-id> --output json
omat issue comment add <issue-id> --help
```
...
For squad leader tasks, OhMyAgentTeam appends a squad leader briefing to the leader
...
members carry no skills segment. Builtin `ohmyagentteam-*` skills are not listed —
...
- recording squad activity with `omat squad activity`;
