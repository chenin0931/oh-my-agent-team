---
name: ohmyagentteam-runtimes-and-repos
description: "Use when inspecting or debugging OhMyAgentTeam runtimes, daemon task claiming, agent not running, workdir/session reuse, or repository checkout. Covers runtime online/offline state, daemon heartbeat/claim chain, task-scoped repo checkout, project repo context, local_directory caveats, and safe diagnostic commands."
user-invocable: false
allowed-tools: Bash(ohmyagentteam *)
---

# OhMyAgentTeam Runtimes and Repos
...
7. `omat repo checkout` talks to the local daemon, not directly to GitHub.

## CLI

```bash
omat runtime list --output json
omat runtime usage <runtime-id> --output json
omat runtime activity <runtime-id> --output json
omat runtime update <runtime-id> --target-version <version> --output json
omat runtime delete <runtime-id>
omat repo checkout <url>
omat repo checkout <url> --ref <branch-or-sha>
```
...
4. Is the runtime online? `omat runtime list --output json`.
