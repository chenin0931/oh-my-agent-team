...
- `server/cmd/omat/cmd_runtime.go` registers `runtime list`, `usage`, `activity`, `update`, and `delete`.
...
- `server/cmd/omat/cmd_repo.go` registers `repo checkout <url> [--ref]`.
- `repo checkout` requires `OMAT_DAEMON_PORT`, sends `workspace_id`, `workdir`, `ref`, `agent_name`, and `task_id` to local daemon `/repo/checkout`, then prints the checked-out path.
