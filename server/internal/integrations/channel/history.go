...
// `omat chat history` (the channel OVERVIEW: top-level messages + thread
// metadata, not thread contents) and `omat chat thread [id]` (one thread's
...
// platform so the agent reads a uniform list, like `omat issue comment list
// --output json`.
type HistoryMessage struct {
	...
	// so the agent can `omat chat thread <thread_id>` to read its contents.
	// They are absent on a plain message and on thread-read rows.

	// ThreadID is the identifier to pass to `omat chat thread <id>` to read
