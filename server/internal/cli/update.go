...
func releaseAssetCandidates(targetVersion, goos, goarch string) []string {
	...
	// `omat_{os}_{arch}` name for releases that still ship it.
	return []string{
		fmt.Sprintf("omat-cli-%s-%s-%s.%s", version, goos, goarch, ext),
		fmt.Sprintf("omat_%s_%s.%s", goos, goarch, ext),
	}
}
...
func fetchReleaseByTag(tag string) (*GitHubRelease, error) {
	...
	req, err := http.NewRequest(http.MethodGet, "https://api.github.com/repos/chenin0931/oh-my-agent-team/releases/tags/"+tag, nil)
	if err != nil {
		return nil, err
	}
...
// FetchLatestRelease fetches the latest release tag from the ohmyagentteam GitHub repo.
func FetchLatestRelease() (*GitHubRelease, error) {
	client := &http.Client{Timeout: 10 * time.Second}
	req, err := http.NewRequest(http.MethodGet, "https://api.github.com/repos/chenin0931/oh-my-agent-team/releases/latest", nil)
	if err != nil {
		return nil, err
	}
	req.Header.Set("Accept", "application/vnd.github+json")
...
// IsBrewInstall checks whether the running ohmyagentteam binary was installed via Homebrew.
...
// UpdateViaBrew runs `brew upgrade chenin0931/tap/ohmyagentteam`.
// Returns the combined output and any error.
func UpdateViaBrew() (string, error) {
	cmd := exec.Command("brew", "upgrade", "chenin0931/tap/ohmyagentteam")
...
func UpdateViaDownloadWithTimeout(targetVersion string, downloadTimeout time.Duration) (string, error) {
	...
	tmpFile, err := os.CreateTemp(dir, "ohmyagentteam-update-*")
	if err != nil {
