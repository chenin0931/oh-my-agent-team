...
const (
	...
	// hard ceiling for cost/resource control can set OMAT_AGENT_TIMEOUT.
	...
	// Set OMAT_AGENT_IDLE_WATCHDOG=0 to disable.
	...
	// that stuck-tool case (MUL-3064). Set OMAT_AGENT_TOOL_WATCHDOG=0 to
...
// OMAT_GC_ARTIFACT_PATTERNS to extend the list per deployment.
...
type Config struct {
	...
	DeviceName                     string
	RuntimeName                    string
	CLIVersion                     string                // ohmyagentteam CLI version (e.g. "0.1.13")
	...
	WorkspacesRoot                 string                // base path for execution envs (default: ~/omat_workspaces)
	...
	AutoUpdateEnabled              bool                  // periodically check for a newer CLI release and self-update when idle (default: true on OhMyAgentTeam Cloud, false on self-host)
	...
	// written by `omat runtime profile set-path`. appendProfileRuntimes
...
func LoadConfig(overrides Overrides) (Config, error) {
	...
	rawServerURL := envOrDefault("OMAT_SERVER_URL", DefaultServerURL)
	...
	//
	//   - OMAT_OPENCLAW_PATH: read by probe() via envOrDefault for the
	...
	// users already get with OMAT_OPENCLAW_PATH today. We achieve it with
	...
	// pinned to an explicit OMAT_*_PATH) must not pay the cost of
	...
	probe := func(envVar, defaultCmd, modelEnv string) (AgentEntry, bool) {
		...
		// who pinned OMAT_*_PATH to an absolute or relative path that
	...
	if e, ok := probe("OMAT_CLAUDE_PATH", "claude", "OMAT_CLAUDE_MODEL"); ok {
		agents["claude"] = e
	}
	if e, ok := probe("OMAT_CODEX_PATH", "codex", "OMAT_CODEX_MODEL"); ok {
		agents["codex"] = e
	}
	if e, ok := probe("OMAT_OPENCODE_PATH", "opencode", "OMAT_OPENCODE_MODEL"); ok {
		agents["opencode"] = e
	}
	if e, ok := probe("OMAT_OPENCLAW_PATH", "openclaw", "OMAT_OPENCLAW_MODEL"); ok {
		agents["openclaw"] = e
	}
	if e, ok := probe("OMAT_HERMES_PATH", "hermes", "OMAT_HERMES_MODEL"); ok {
		agents["hermes"] = e
	}
	if e, ok := probe("OMAT_PI_PATH", "pi", "OMAT_PI_MODEL"); ok {
		agents["pi"] = e
	}
	if e, ok := probe("OMAT_CURSOR_PATH", "cursor-agent", "OMAT_CURSOR_MODEL"); ok {
		agents["cursor"] = e
	}
	if e, ok := probe("OMAT_COPILOT_PATH", "copilot", "OMAT_COPILOT_MODEL"); ok {
		agents["copilot"] = e
	}
	if e, ok := probe("OMAT_KIMI_PATH", "kimi", "OMAT_KIMI_MODEL"); ok {
		agents["kimi"] = e
	}
	if e, ok := probe("OMAT_KIRO_PATH", "kiro-cli", "OMAT_KIRO_MODEL"); ok {
		agents["kiro"] = e
	}
	if e, ok := probe("OMAT_CODEBUDDY_PATH", "codebuddy", "OMAT_CODEBUDDY_MODEL"); ok {
		agents["codebuddy"] = e
	}
	// agy 1.0.6 added a `--model` flag (MUL-3125), so Antigravity now takes a
	// model env like every other backend. OMAT_ANTIGRAVITY_MODEL seeds the
	...
	if e, ok := probe("OMAT_ANTIGRAVITY_PATH", "agy", "OMAT_ANTIGRAVITY_MODEL"); ok {
		agents["antigravity"] = e
	}
	qoderPath := envOrDefault("OMAT_QODER_PATH", "qodercli")
	if path, err := resolveAgentExecutablePath(qoderPath); err == nil {
		agents["qoder"] = AgentEntry{
			Path:  path,
			Model: strings.TrimSpace(os.Getenv("OMAT_QODER_MODEL")),
		}
	}
	// ByteDance official TRAE CLI (the `traecli` binary from https://docs.trae.cn/cli),
	// driven over ACP via `traecli acp serve --yolo`. OMAT_TRAECLI_MODEL seeds
	// the daemon-wide default model (a model id from the user's logged-in traecli
	// catalog).
	if e, ok := probe("OMAT_TRAECLI_PATH", "traecli", "OMAT_TRAECLI_MODEL"); ok {
		agents["traecli"] = e
	}
	if len(agents) == 0 {
	...
	claudeArgs, err := shellArgsFromEnv("OMAT_CLAUDE_ARGS")
	if err != nil {
		return Config{}, err
	}
	codexArgs, err := shellArgsFromEnv("OMAT_CODEX_ARGS")
	if err != nil {
		return Config{}, err
	}
	codebuddyArgs, err := shellArgsFromEnv("OMAT_CODEBUDDY_ARGS")
	if err != nil {
		return Config{}, err
	}
	...
	pollInterval, err := durationFromEnv("OMAT_DAEMON_POLL_INTERVAL", DefaultPollInterval)
	if err != nil {
		return Config{}, err
	}
	...
	heartbeatInterval, err := durationFromEnv("OMAT_DAEMON_HEARTBEAT_INTERVAL", DefaultHeartbeatInterval)
	if err != nil {
		return Config{}, err
	}
	...
	agentTimeout, err := durationFromEnv("OMAT_AGENT_TIMEOUT", DefaultAgentTimeout)
	if err != nil {
		return Config{}, err
	}
	...
	codexSemanticInactivityTimeout, err := durationFromEnv("OMAT_CODEX_SEMANTIC_INACTIVITY_TIMEOUT", DefaultCodexSemanticInactivityTimeout)
	if err != nil {
		return Config{}, err
	}
	...
	// OMAT_AGENT_IDLE_WATCHDOG=0 disables the per-task idle watchdog. We
	...
	agentIdleWatchdog, err := durationFromEnv("OMAT_AGENT_IDLE_WATCHDOG", DefaultAgentIdleWatchdog)
	if err != nil {
		return Config{}, err
	}

	// OMAT_AGENT_TOOL_WATCHDOG=0 disables the in-flight-tool backstop; any
	// positive duration overrides DefaultAgentToolWatchdog.
	agentToolWatchdog, err := durationFromEnv("OMAT_AGENT_TOOL_WATCHDOG", DefaultAgentToolWatchdog)
	if err != nil {
		return Config{}, err
	}

	maxConcurrentTasks, err := intFromEnv("OMAT_DAEMON_MAX_CONCURRENT_TASKS", DefaultMaxConcurrentTasks)
	if err != nil {
		return Config{}, err
	}
	...
	// Callers may still pin a specific id via OMAT_DAEMON_ID or the
	// override field (e.g. for tests or embedded environments).
	daemonID := strings.TrimSpace(os.Getenv("OMAT_DAEMON_ID"))
	...
	// — e.g. ~/.ohmyagentteam/daemon.id (default) plus ~/.ohmyagentteam/profiles/<x>/
	...
	// the user explicitly pins OMAT_DAEMON_ID=<hostname>, or when the
	// canonical id was itself promoted from a pre-change profile file).
	legacyDaemonIDs = filterLegacyIDs(legacyDaemonIDs, daemonID)

	deviceName := envOrDefault("OMAT_DAEMON_DEVICE_NAME", host)
	if overrides.DeviceName != "" {
		deviceName = overrides.DeviceName
	}

	runtimeName := envOrDefault("OMAT_AGENT_RUNTIME_NAME", DefaultRuntimeName)
	if overrides.RuntimeName != "" {
		runtimeName = overrides.RuntimeName
	}

	// Workspaces root: override > env > default (~/omat_workspaces or ~/omat_workspaces_<profile>)
	...
	keepEnv := os.Getenv("OMAT_KEEP_ENV_AFTER_TASK") == "true" || os.Getenv("OMAT_KEEP_ENV_AFTER_TASK") == "1"

	// GC config: env > defaults
	gcEnabled := true
	if v := os.Getenv("OMAT_GC_ENABLED"); v == "false" || v == "0" {
		gcEnabled = false
	}
	gcInterval, err := durationFromEnv("OMAT_GC_INTERVAL", DefaultGCInterval)
	if err != nil {
		return Config{}, err
	}
	gcTTL, err := durationFromEnv("OMAT_GC_TTL", DefaultGCTTL)
	if err != nil {
		return Config{}, err
	}
	gcOrphanTTL, err := durationFromEnv("OMAT_GC_ORPHAN_TTL", DefaultGCOrphanTTL)
	if err != nil {
		return Config{}, err
	}
	gcArtifactTTL, err := durationFromEnv("OMAT_GC_ARTIFACT_TTL", DefaultGCArtifactTTL)
	if err != nil {
		return Config{}, err
	}
	gcArtifactPatterns := patternsFromEnv("OMAT_GC_ARTIFACT_PATTERNS", DefaultGCArtifactPatterns)

	// Auto-update config: default -> env override -> CLI override.
	//
	// Default is opt-in on OhMyAgentTeam Cloud (api.ohmyagentteam.com) and opt-out for
	...
	// Operators on either side can flip the default with OMAT_DAEMON_AUTO_UPDATE.
	autoUpdateEnabled := isOfficialCloudServer(serverBaseURL)
	if v := strings.TrimSpace(os.Getenv("OMAT_DAEMON_AUTO_UPDATE")); v != "" {
	...
	autoUpdateInterval, err := durationFromEnv("OMAT_DAEMON_AUTO_UPDATE_INTERVAL", DefaultAutoUpdateCheckInterval)
	if err != nil {
		return Config{}, err
	}
...
// officialCloudHost is the hostname of OhMyAgentTeam's hosted cloud. It's the only
// origin we treat as "official" for the auto-update default — staging,
// preview, and any future *.ohmyagentteam.com subdomains are deliberately excluded
// so they inherit the safer self-host default until explicitly opted in.
const officialCloudHost = "api.ohmyagentteam.com"

// isOfficialCloudServer reports whether the resolved server base URL points
// at OhMyAgentTeam's hosted cloud. Used to pick the auto-update default: cloud
...
func NormalizeServerBaseURL(raw string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("invalid OMAT_SERVER_URL: %w", err)
	}
	switch u.Scheme {
	case "ws":
		u.Scheme = "http"
	...
	case "http", "https":
	default:
		return "", fmt.Errorf("OMAT_SERVER_URL must use ws, wss, http, or https")
	}
	if u.Path == "/ws" {
		u.Path = ""
	}
...
// OMAT_WORKSPACES_ROOT env > default ($HOME/omat_workspaces, or
// $HOME/omat_workspaces_<profile> for a named profile). Read-only callers
// (e.g. `omat daemon disk-usage`) use this directly so they pick the same
// directory the running daemon would have picked.
func ResolveWorkspacesRoot(profile, override string) (string, error) {
	root := strings.TrimSpace(os.Getenv("OMAT_WORKSPACES_ROOT"))
	if override != "" {
		root = override
	}
	if root == "" {
		...
		if err != nil {
			return "", fmt.Errorf("resolve home directory: %w (set OMAT_WORKSPACES_ROOT to override)", err)
		}
		if profile != "" {
			root = filepath.Join(home, "omat_workspaces_"+profile)
		} else {
			root = filepath.Join(home, "omat_workspaces")
		}
	}
...
func ArtifactPatternsFromEnv() []string {
	return patternsFromEnv("OMAT_GC_ARTIFACT_PATTERNS", DefaultGCArtifactPatterns)
}
...
// When ~/.ohmyagentteam/hooks shadows a real agent binary, skip that hooks directory:
...
func resolveAgentExecutablePath(cmd string) (string, error) {
	...
	if isInOhMyAgentTeamHooksDir(resolved) {
		if unshadowed, err := lookPathExcludingOhMyAgentTeamHooks(cmd); err == nil {
...
func lookPathExcludingOhMyAgentTeamHooks(cmd string) (string, error) {
	for _, dir := range filepath.SplitList(os.Getenv("PATH")) {
		if dir == "" {
			dir = "."
		}
		if isOhMyAgentTeamHooksDir(dir) {
			continue
		}
...
func isInOhMyAgentTeamHooksDir(path string) bool {
	if path == "" {
		return false
	}
	return isOhMyAgentTeamHooksDir(filepath.Dir(path))
}

func isOhMyAgentTeamHooksDir(dir string) bool {
	home, err := os.UserHomeDir()
	if err != nil || home == "" {
		return false
	}
	return samePathDir(dir, filepath.Join(home, ".ohmyagentteam", "hooks"))
}
...
// before any OMAT_*_PATH override is applied. Kept in sync with the
...
//     keep the script readable. Custom OMAT_*_PATH values never reach this
...
//  6. if the resolved path lives in ~/.ohmyagentteam/hooks, searches the same
...
func buildLoginShellResolveScript(names []string) string {
	...
	b.WriteString("  hc=\"\"\n")
	b.WriteString("  if [ -n \"${HOME:-}\" ]; then hd=\"$HOME/.ohmyagentteam/hooks\"; hc=$(cd \"$hd\" 2>/dev/null && pwd -P) || hc=\"\"; fi\n")
...
//
//   - The two vars touched (OMAT_OPENCLAW_PATH, OPENCLAW_STATE_DIR) are
...
func applyOpenclawOverride(oc *cli.OpenClawOverride) {
	...
	if oc.BinaryPath != "" {
		if _, set := os.LookupEnv("OMAT_OPENCLAW_PATH"); !set {
			_ = os.Setenv("OMAT_OPENCLAW_PATH", oc.BinaryPath)
		}
	}
	if oc.StateDir != "" {
