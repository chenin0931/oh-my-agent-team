...
// same machine shares one UUID stored at `~/.ohmyagentteam/daemon.id`. Profile
...
// LegacyDaemonUUIDs scans `~/.ohmyagentteam/profiles/*/daemon.id` and returns every
...
// explicitly pins OMAT_DAEMON_ID to the hostname itself, there's nothing
