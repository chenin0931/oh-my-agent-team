...
// — users cannot audit or edit the contents from any OhMyAgentTeam UI.
//
// This conflicts with the OhMyAgentTeam daemon's context model. OhMyAgentTeam
...
//     unrelated host-project context into OhMyAgentTeam tasks. The reproduction
//     in github.com/chenin0931/oh-my-agent-team#3130 saw Raw Memories from
//     `D:\Project\MoHaYu\WowChat` (a host-local project) injected into a
//     brand-new OhMyAgentTeam issue's first Codex turn.
//
...
//
// Users who explicitly want Codex native memory inside a OhMyAgentTeam task
// (and accept the leak risk) can keep the feature enabled by setting
// `OMAT_CODEX_MEMORY=1` in the daemon environment. The long-term
// answer for durable agent context is a OhMyAgentTeam-owned, user-visible,
...
// OhMyAgentTeamCodexMemoryEnv is the env var users can set to keep Codex
...
const OhMyAgentTeamCodexMemoryEnv = "OMAT_CODEX_MEMORY"

const (
	omatMemoryFeatureBeginMarker = "# BEGIN ohmyagentteam-managed memory-feature (do not edit; regenerated by daemon)"
	omatMemoryFeatureEndMarker   = "# END ohmyagentteam-managed memory-feature"
	omatMemoryConfigBeginMarker  = "# BEGIN ohmyagentteam-managed memory-config (do not edit; regenerated by daemon)"
	omatMemoryConfigEndMarker    = "# END ohmyagentteam-managed memory-config"
)
...
var memoryFeatureBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(omatMemoryFeatureBeginMarker) +
		`.*?^` + regexp.QuoteMeta(omatMemoryFeatureEndMarker) + `\n*`)

var memoryConfigBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(omatMemoryConfigBeginMarker) +
		`.*?^` + regexp.QuoteMeta(omatMemoryConfigEndMarker) + `\n*`)
...
func codexMemoryEnabled() bool {
	raw := strings.TrimSpace(os.Getenv(OhMyAgentTeamCodexMemoryEnv))
...
// renderOhMyAgentTeamMemoryFeatureBlock returns the daemon-managed memory-feature
// block. The body uses `memories = false` when injected inside a
// `[features]` table, and `features.memories = false` otherwise.
func renderOhMyAgentTeamMemoryFeatureBlock(inFeaturesTable bool) string {
	var b strings.Builder
	b.WriteString(omatMemoryFeatureBeginMarker)
	b.WriteString("\n")
	if inFeaturesTable {
	...
	b.WriteString(omatMemoryFeatureEndMarker)
	b.WriteString("\n")
	return b.String()
}

// renderOhMyAgentTeamMemoryConfigBlock returns the daemon-managed memory-config
// block. The body uses bare keys when injected inside a `[memories]` table
// and dotted-key form otherwise.
func renderOhMyAgentTeamMemoryConfigBlock(inMemoriesTable bool) string {
	var b strings.Builder
	b.WriteString(omatMemoryConfigBeginMarker)
	b.WriteString("\n")
	if inMemoriesTable {
	...
	b.WriteString(omatMemoryConfigEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
func injectMemoryFeatureBlockIntoFeaturesTable(content string) string {
	return injectAfterHeader(content, rootFeaturesTableHeaderRe, renderOhMyAgentTeamMemoryFeatureBlock(true))
}
...
func injectMemoryConfigBlockIntoMemoriesTable(content string) string {
	return injectAfterHeader(content, rootMemoriesTableHeaderRe, renderOhMyAgentTeamMemoryConfigBlock(true))
}
...
//
// When OMAT_CODEX_MEMORY is set to a truthy value, the function is a
...
func ensureCodexMemoryConfig(configPath string, logger *slog.Logger) error {
	if codexMemoryEnabled() {
		if logger != nil {
			logger.Info("codex memory: leaving Codex native memory untouched per OMAT_CODEX_MEMORY",
	...
	if !hasFeatures {
		prepend.WriteString(renderOhMyAgentTeamMemoryFeatureBlock(false))
	}
	if !hasMemories {
		prepend.WriteString(renderOhMyAgentTeamMemoryConfigBlock(false))
	}
