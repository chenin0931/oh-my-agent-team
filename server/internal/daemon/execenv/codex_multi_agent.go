...
// subagents. The OhMyAgentTeam daemon currently models only the parent Codex
...
// lifecycle state with drain/cancel primitives, or the OhMyAgentTeam runtime
...
//
// Users who explicitly want Codex native subagents inside a OhMyAgentTeam task
// (and accept the lifecycle risk) can keep the feature enabled by setting
// `OMAT_CODEX_MULTI_AGENT=1` in the daemon environment.
//
// Layout note
//
...
// OhMyAgentTeamCodexMultiAgentEnv is the env var users can set to keep Codex
...
const OhMyAgentTeamCodexMultiAgentEnv = "OMAT_CODEX_MULTI_AGENT"

// omatMultiAgentBeginMarker / omatMultiAgentEndMarker delimit the
// multi-agent-specific managed block. Kept separate from the sandbox
// block so each can evolve and migrate independently.
const (
	omatMultiAgentBeginMarker = "# BEGIN ohmyagentteam-managed multi-agent (do not edit; regenerated by daemon)"
	omatMultiAgentEndMarker   = "# END ohmyagentteam-managed multi-agent"
)
...
var multiAgentBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(omatMultiAgentBeginMarker) +
		`.*?^` + regexp.QuoteMeta(omatMultiAgentEndMarker) + `\n*`)
...
func codexMultiAgentEnabled() bool {
	raw := strings.TrimSpace(os.Getenv(OhMyAgentTeamCodexMultiAgentEnv))
...
// renderOhMyAgentTeamMultiAgentBlock returns the daemon-managed multi-agent
// block. The body uses `multi_agent = false` when injected inside a
// `[features]` table, and `features.multi_agent = false` otherwise.
func renderOhMyAgentTeamMultiAgentBlock(inFeaturesTable bool) string {
	var b strings.Builder
	b.WriteString(omatMultiAgentBeginMarker)
	b.WriteString("\n")
	if inFeaturesTable {
	...
	b.WriteString(omatMultiAgentEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
func injectManagedBlockIntoFeaturesTable(content string) string {
	block := renderOhMyAgentTeamMultiAgentBlock(true)
...
//
// When OMAT_CODEX_MULTI_AGENT is set to a truthy value, the function is
...
func ensureCodexMultiAgentConfig(configPath string, logger *slog.Logger) error {
	if codexMultiAgentEnabled() {
		if logger != nil {
			logger.Info("codex multi-agent: leaving Codex native multi-agent untouched per OMAT_CODEX_MULTI_AGENT",
	...
	if hasRootFeaturesTable(existing) {
		...
		block := renderOhMyAgentTeamMultiAgentBlock(false)
		if existing == "" {
			updated = block
		} else {
			updated = block + "\n" + existing
