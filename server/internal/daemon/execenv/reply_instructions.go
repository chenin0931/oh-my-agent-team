	...
	if threadID != "" {
		return fmt.Sprintf(
			"%d new comment(s) on this issue since your last run — don't read them all blindly. "+
				...
				"`omat issue comment list %s --thread %s --since %s --output json` "+
				...
				"`omat issue comment list %s --since %s --output json`.\n\n",
...
func BuildResumedCommentsHint(issueID, triggerCommentID, triggerThreadID string) string {
	...
	return fmt.Sprintf(
		"You're resuming the prior session, and the triggering comment is already included above. "+
			...
			"`omat issue comment list %s --thread %s --tail 30 --output json`.\n\n",
...
func BuildColdCommentsHint(issueID, triggerCommentID, triggerThreadID string) string {
	...
	return fmt.Sprintf(
		"Read the triggering conversation first: "+
			...
			"Need cross-thread background? `omat issue comment list %s --recent 10 --output json` "+
...
//     `?` before the bytes reach `omat.exe` (#2198 Chinese, #2236
//     Chinese, #2376 Cyrillic).
//     2. On any host, when the model emits a multi-flag command (e.g.
//     `omat issue create --title ... --assignee-id ... --project ...`)
...
func BuildCommentReplyInstructions(provider, issueID, triggerCommentID string) string {
	...
	if runtimeGOOS == "windows" {
		return fmt.Sprintf(
			"If you decide to reply, post it as a comment — always use the trigger comment ID below, "+
				...
				"Do NOT pipe via `--content-stdin` — Windows PowerShell 5.1's `$OutputEncoding` defaults to ASCIIEncoding when piping to native commands and silently drops non-ASCII (Chinese, Japanese, Cyrillic, accents, emoji) as `?` before the bytes reach `omat.exe`. "+
				...
				"    # 2. Post the comment:\n"+
				"    omat issue comment add %s --parent %s --content-file ./reply.md\n"+
	...
	return fmt.Sprintf(
		"If you decide to reply, post it as a comment — always use the trigger comment ID below, "+
			...
			"Do NOT use `--content-stdin` with a HEREDOC either — when extra flags (e.g. `--assignee`, `--project` on `omat issue create`) accompany the command, the bash heredoc/flag boundary is fragile and flags can be silently swallowed into the stdin stream while the command still exits 0 (see GitHub #4182, OXY-78 / OXY-76). "+
	...
	if runtimeGOOS == "windows" {
		return fmt.Sprintf(
			"If you decide to reply, post it as a comment — always use the trigger comment ID below, "+
				...
				"Do NOT pipe via `--content-stdin` — PowerShell 5.1's `$OutputEncoding` defaults to ASCIIEncoding when piping to native commands and silently drops non-ASCII (Chinese, Japanese, Cyrillic, accents, emoji) as `?` before bytes reach `omat.exe`. "+
