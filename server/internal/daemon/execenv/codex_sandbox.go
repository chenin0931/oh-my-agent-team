...
// `no such host` errors when calling out (for example, `omat issue get`
// hitting the OhMyAgentTeam API). See upstream issue openai/codex#10390.
//
...
// actually reach the OhMyAgentTeam API. On Linux (and on macOS once the upstream
...
//   danger-full-access so the OhMyAgentTeam CLI can reach the API.
...
// omatManagedBeginMarker / omatManagedEndMarker delimit the block the
...
const (
	omatManagedBeginMarker = "# BEGIN ohmyagentteam-managed (do not edit; regenerated by daemon)"
	omatManagedEndMarker   = "# END ohmyagentteam-managed"
)

// renderOhMyAgentTeamManagedBlock produces the managed block for the given policy.
//
...
//     other table (e.g. `[permissions.ohmyagentteam]`), a bare `sandbox_mode = ...`
...
// at the top of the file (see upsertOhMyAgentTeamManagedBlock), avoids both traps.
func renderOhMyAgentTeamManagedBlock(policy codexSandboxPolicy) string {
	var b strings.Builder
	b.WriteString(omatManagedBeginMarker)
	b.WriteString("\n")
	b.WriteString(fmt.Sprintf("sandbox_mode = %q\n", policy.Mode))
	if policy.Mode == "workspace-write" {
		b.WriteString(fmt.Sprintf("sandbox_workspace_write.network_access = %t\n", policy.NetworkAccess))
	}
	b.WriteString(omatManagedEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
var managedBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(omatManagedBeginMarker) +
		`.*?^` + regexp.QuoteMeta(omatManagedEndMarker) + `\n*`)

// upsertOhMyAgentTeamManagedBlock returns the config content with the ohmyagentteam-managed
...
// `[permissions.ohmyagentteam]` or `[profiles.foo]`. Combined with the dotted-key
// form used by renderOhMyAgentTeamManagedBlock, this means the managed block neither
// leaks into nor inherits from any surrounding table scope.
func upsertOhMyAgentTeamManagedBlock(content string, policy codexSandboxPolicy) string {
	// Drop any previously written managed block (wherever it sits).
	content = managedBlockRe.ReplaceAllString(content, "")
	block := renderOhMyAgentTeamManagedBlock(policy)
...
// ensureCodexSandboxConfig writes the ohmyagentteam-managed sandbox block into the
...
func ensureCodexSandboxConfig(configPath string, policy codexSandboxPolicy, detectedVersion string, logger *slog.Logger) error {
	...
	updated := upsertOhMyAgentTeamManagedBlock(existing, policy)
