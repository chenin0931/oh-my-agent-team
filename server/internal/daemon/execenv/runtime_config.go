...
// runtimeMarkerBegin and runtimeMarkerEnd delimit the OhMyAgentTeam-managed brief
...
const (
	runtimeMarkerBegin = "<!-- BEGIN OMAT-RUNTIME (auto-managed; do not edit) -->"
	runtimeMarkerEnd   = "<!-- END OMAT-RUNTIME -->"
...
// writeRuntimeConfigFile writes the OhMyAgentTeam runtime brief to path without
...
// locateMarkerBlock finds the [start, end) byte range of the OhMyAgentTeam marker
...
//
//   - User content carries a stray `<!-- END OMAT-RUNTIME -->` (e.g. a
...
// CleanupRuntimeConfig excises the OhMyAgentTeam marker block from the runtime
...
// of OhMyAgentTeam runs without ever touching a single non-managed byte.
//
...
// the user inside the same directory after a OhMyAgentTeam task would pick up
...
// about the OhMyAgentTeam runtime environment and available CLI tools.
//
...
func buildMetaSkillContent(provider string, ctx TaskContextForEnv) string {
	...
	if ctx.MemberAssigneeAdvisor {
		return buildMetaSkillContentAdvisor(provider, ctx)
	}
	var b strings.Builder
	...
	// the agent's own OhMyAgentTeam credentials (always owner-scoped). For a
	...
	if safeInitiator := sanitizeNameForBriefMarkdown(ctx.InitiatorName); safeInitiator != "" {
		...
		b.WriteString("Note: this is an attested identity for your own routing and privacy logic. Your OhMyAgentTeam credentials stay scoped to the runtime owner, so the initiator's identity does not by itself widen or narrow what you can read or write — do not assume the initiator can see everything you can.\n\n")
	}
	...
	b.WriteString("- `omat issue update <id> [--title X] [--description X | --description-file <path> | --description-stdin] [--priority X] [--status X] [--assignee X | --assignee-id <uuid>] [--parent <issue-id>] [--stage N] [--project <project-id>] [--due-date <RFC3339>]` — Update issue fields; use `--parent \"\"` to clear parent. For agent-authored long descriptions, prefer `--description-file <path>` over stdin (#4182).\n")
	b.WriteString("- `omat repo checkout <url> [--ref <branch-or-sha>]` — Check out a repository into the working directory (creates a git worktree with a dedicated branch; use `--ref` for review/QA on a specific branch, tag, or commit)\n")
	b.WriteString("- `omat issue status <id> <status>` — Shortcut for `issue update --status` when you only need to flip status (todo, in_progress, in_review, done, blocked, backlog, cancelled)\n")
	b.WriteString("- `omat issue children <id> [--output json]` — List a parent's sub-issues grouped by stage (table or JSON), so you can see how many children there are, which stage each is in, and which stage to promote next.\n")
	// Available Commands lists `omat issue comment add` with all three input
	...
	b.WriteString("- `omat issue comment add <issue-id> [--content \"...\" | --content-file <path> | --content-stdin] [--parent <comment-id>] [--attachment <path>]` — Post a comment. For agent-authored bodies, **write the body to a UTF-8 file and use `--content-file <path>`** — do NOT inline `--content` (the shell rewrites backticks, `$()`, quotes, or newlines before the CLI sees them) and do NOT use `--content-stdin` with a HEREDOC (extra flags around the heredoc can be silently swallowed, #4182). See ## Comment Formatting below. Run `omat issue comment add --help` for details.\n")
	b.WriteString("- `omat issue metadata list <issue-id> [--output json]` — List every metadata key pinned to an issue. Empty `{}` is normal.\n")
	b.WriteString("- `omat issue metadata set <issue-id> --key <k> --value <v> [--type string|number|bool]` — Pin (or overwrite) a single metadata key. The CLI auto-infers JSON primitives, so URLs and plain text are stored as strings — pass `--type number` or `--type bool` only when the semantic type matters.\n")
	b.WriteString("- `omat issue metadata delete <issue-id> --key <k>` — Remove a metadata key.\n\n")
	b.WriteString("### Squad maintenance\n")
	b.WriteString("- `omat squad member set-role <squad-id> --member-id <id> --member-type <agent|member> --role <role> [--output json]` — Change a squad member role in place; use this instead of remove+add when only the role changes.\n\n")
	...
	//      non-ASCII as `?` before the bytes reach `omat.exe` (#2198 /
	//      #2236 / #2376); (b) on any host, when the model emits a multi-flag
	//      command (`omat issue create --title ... --assignee-id ...
	...
	if runtimeGOOS == "windows" {
		b.WriteString("On Windows, **always write the comment body to a UTF-8 file with your file-write tool first, then post it with `--content-file <path>`** — do NOT pipe via `--content-stdin`. PowerShell 5.1's `$OutputEncoding` defaults to ASCIIEncoding when piping to a native command, silently dropping non-ASCII characters as `?` before they reach `omat.exe`. Never use inline `--content` for agent-authored comments. ")
	...
	if len(ctx.Repos) > 0 {
		...
		b.WriteString("Use `omat repo checkout <url>` to check out a repository into your working directory. Add `--ref <branch-or-sha>` when you need an exact branch, tag, or commit.\n\n")
	...
	// The full structured payload is also available at .ohmyagentteam/project/resources.json
	...
	if ctx.ChatSessionID != "" {
		...
		// have a single source of truth. Quick-create is issue-less, so the
		...
		if ctx.QuickCreateMode == "planning" {
			...
			b.WriteString("- If any create command returns an error, stop immediately and exit with that error as the only output. Do not retry.\n\n")
		} else {
	...
	b.WriteString("- `[MUL-99](mention://epic/<epic-id>)` — clickable link to an Epic planning container (safe, no side effect)\n")
	...
	b.WriteString("All interactions with OhMyAgentTeam platform resources — including issues, comments, attachments, images, files, and any other platform data — **must** go through the `ohmyagentteam` CLI. ")
	b.WriteString("Do NOT use `curl`, `wget`, or any other HTTP client to access OhMyAgentTeam URLs or APIs directly. ")
	b.WriteString("OhMyAgentTeam resource URLs require authenticated access that only the `ohmyagentteam` CLI can provide.\n\n")
	b.WriteString("If you need to perform an operation that is not covered by any existing `ohmyagentteam` command, ")
...
func writeBackgroundTaskSafetyInstructions(b *strings.Builder) {
	...
	b.WriteString("OhMyAgentTeam marks this task terminal the moment your top-level agent process/turn exits. A OhMyAgentTeam-managed run has NO \"background work finishes later and wakes you up\" step: whatever you leave running in the background is orphaned, its result is lost, and the final comment you meant to post once it finished never sends. The user sees a task that ended with no conclusion and has to re-trigger you.\n\n")
	...
	b.WriteString("- If a tool response says to wait for a future notification/reminder, or tells you it is now running in the background so you can keep working, do NOT rely on that in OhMyAgentTeam-managed runs — that hint comes from the standalone harness and does not apply here. Block on the appropriate wait/output/collect operation before exiting.\n")
