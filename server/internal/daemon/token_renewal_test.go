...
func TestTryRenewToken_SurfacesReloginWarningOn401(t *testing.T) {
	...
	if !strings.Contains(out, "omat login") {
		t.Fatalf("401 warning must tell the user to run 'omat login', got: %s", out)
	}
}
...
// and the operator would never see a "run omat login" hint.
func TestPreflightAuth_RenewsBeforeWorkspaceSyncOnExpiredToken(t *testing.T) {
	...
	if !strings.Contains(out, "omat login") {
		t.Fatalf("expected the actionable 'run omat login' hint in the WARN, got: %s", out)
	}
}
