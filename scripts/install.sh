#!/usr/bin/env bash
# OhMyAgentTeam installer — installs the CLI and optionally provisions a self-host server.
#
...
# Configuration
# ---------------------------------------------------------------------------
REPO_URL="https://github.com/chenin0931/oh-my-agent-team.git"
REPO_WEB_URL="https://github.com/chenin0931/oh-my-agent-team"  # without .git, for GitHub web APIs
INSTALL_DIR="${OMAT_INSTALL_DIR:-$HOME/.ohmyagentteam/server}"
BREW_PACKAGE="chenin0931/tap/omat"
...
detect_os() {
  ...
  irm https://raw.githubusercontent.com/chenin0931/oh-my-agent-team/main/scripts/install.ps1 | iex" ;;
    *)      fail "Unsupported operating system: $(uname -s). OhMyAgentTeam supports macOS, Linux, and Windows." ;;
  esac
...
install_cli_brew() {
  info "Installing OhMyAgentTeam CLI via Homebrew..."
  local brew_log
  brew_log=$(mktemp)
  if ! brew tap chenin0931/tap >"$brew_log" 2>&1; then
  ...
  if ! brew install "$BREW_PACKAGE" >"$brew_log" 2>&1; then
    if brew list "$BREW_PACKAGE" >/dev/null 2>&1; then
      rm -f "$brew_log"
      ok "OhMyAgentTeam CLI already installed via Homebrew"
    else
      warn "Failed to install omat via Homebrew. Falling back to GitHub Releases binary install."
      _dump_brew_log "$brew_log"
      rm -f "$brew_log"
      return 1
    fi
    ...
    rm -f "$brew_log"
    ok "OhMyAgentTeam CLI installed via Homebrew"
  fi
}

install_cli_binary() {
  info "Installing OhMyAgentTeam CLI from GitHub Releases..."
  ...
  local url="https://github.com/chenin0931/oh-my-agent-team/releases/download/${latest}/omat-cli-${version}-${OS}-${ARCH}.tar.gz"
  local tmp_dir
  tmp_dir=$(mktemp -d)

  info "Downloading $url ..."
  if ! curl -fsSL "$url" -o "$tmp_dir/omat.tar.gz"; then
    rm -rf "$tmp_dir"
    fail "Failed to download CLI binary."
  fi

  tar -xzf "$tmp_dir/omat.tar.gz" -C "$tmp_dir" omat

  # Try /usr/local/bin first, fall back to ~/.local/bin. Tests and scripted
  # installs can override the first choice with OMAT_BIN_DIR.
  local bin_dir="${OMAT_BIN_DIR:-/usr/local/bin}"
  if [ -w "$bin_dir" ]; then
    mv "$tmp_dir/omat" "$bin_dir/omat"
  elif command_exists sudo; then
    sudo mv "$tmp_dir/omat" "$bin_dir/omat"
  else
    bin_dir="$HOME/.local/bin"
    mkdir -p "$bin_dir"
    mv "$tmp_dir/omat" "$bin_dir/omat"
    chmod +x "$bin_dir/omat"
  ...
  rm -rf "$tmp_dir"
  ok "OhMyAgentTeam CLI installed to $bin_dir/omat"
}
...
add_to_path() {
  ...
  for rc in "$HOME/.bashrc" "$HOME/.zshrc"; do
    if [ -f "$rc" ] && ! grep -qF "$dir" "$rc"; then
      printf '\n# Added by OhMyAgentTeam installer\n%s\n' "$line" >> "$rc"
    fi
  done
}
...
get_selfhost_ref() {
  if [ -n "${OMAT_SELFHOST_REF:-}" ]; then
    printf '%s' "$OMAT_SELFHOST_REF"
    return
  fi
...
upgrade_cli_brew() {
  info "Upgrading OhMyAgentTeam CLI via Homebrew..."
  brew update 2>/dev/null || true
  if brew upgrade "$BREW_PACKAGE" 2>/dev/null; then
    ok "OhMyAgentTeam CLI upgraded via Homebrew"
  else
    # brew upgrade exits non-zero if already up to date
    ok "OhMyAgentTeam CLI is already the latest version"
  fi
}

install_cli() {
  if command_exists omat; then
    local current_ver
    # `omat version` outputs "omat 0.3.23 (commit: f46b929eb, built: 2026-06-16T10:11:56Z)".
    current_ver=$(omat version 2>/dev/null | awk 'NR==1{print $2}' || echo "unknown")
    ...
    if [ -z "$latest_ver" ] || [ "$current_cmp" = "$latest_cmp" ]; then
      ok "OhMyAgentTeam CLI is up to date ($current_ver)"
      return 0
    fi

    info "OhMyAgentTeam CLI $current_ver installed, latest is $latest_ver — upgrading..."
    ...
    local new_ver
    new_ver=$(omat version 2>/dev/null | awk 'NR==1{print $2}' || echo "unknown")
    ok "OhMyAgentTeam CLI upgraded ($current_ver → $new_ver)"
    return 0
  fi
  ...
  # Verify
  if ! command_exists omat; then
    fail "CLI installed but 'omat' was not found on PATH. You may need to restart your shell."
  fi
}
...
check_docker() {
  if ! command_exists docker; then
    printf "\n"
    fail "Docker is not installed. OhMyAgentTeam self-hosting requires Docker and Docker Compose.
...
# ---------------------------------------------------------------------------
setup_server() {
  info "Setting up OhMyAgentTeam server..."
  local server_ref
  ...
  if [ -d "$INSTALL_DIR/.git" ]; then
    ...
    info "Cloning OhMyAgentTeam repository..."
  ...
  info "Pulling official OhMyAgentTeam images..."
  pull_official_selfhost_images
  info "Starting OhMyAgentTeam services (this may take a few minutes on first run)..."
  ...
  if [ "$ready" = true ]; then
    ok "OhMyAgentTeam server is running"
  else
...
run_default() {
  printf "\n"
  printf "${BOLD}  OhMyAgentTeam — Installer${RESET}\n"
  printf "\n"
  ...
  printf "${BOLD}${GREEN}  ✓ OhMyAgentTeam CLI is ready!${RESET}\n"
  printf "${BOLD}${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}\n"
  printf "\n"
  printf "  ${BOLD}Next: configure your environment${RESET}\n"
  printf "\n"
  printf "     ${CYAN}omat setup${RESET}                # Connect this computer\n"
  printf "     ${CYAN}omat setup self-host${RESET}       # Connect to a self-hosted server\n"
  printf "\n"
  printf "  ${BOLD}Self-hosting?${RESET} Install the server first:\n"
  printf "     curl -fsSL https://raw.githubusercontent.com/chenin0931/oh-my-agent-team/main/scripts/install.sh | bash -s -- --with-server\n"
  printf "\n"
}
...
run_with_server() {
  printf "\n"
  printf "${BOLD}  OhMyAgentTeam — Self-Host Installer${RESET}\n"
  ...
  printf "\n"
  printf "${BOLD}${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}\n"
  printf "${BOLD}${GREEN}  ✓ OhMyAgentTeam server is running and CLI is ready!${RESET}\n"
  printf "${BOLD}${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}\n"
  printf "\n"
  ...
  printf "\n"
  printf "     ${CYAN}omat setup self-host${RESET}   # Configure + authenticate + start daemon\n"
  printf "\n"
  ...
  printf "     curl -fsSL https://raw.githubusercontent.com/chenin0931/oh-my-agent-team/main/scripts/install.sh | bash -s -- --stop\n"
  printf "\n"
}
...
run_stop() {
  printf "\n"
  info "Stopping OhMyAgentTeam services..."
  ...
  if [ -d "$INSTALL_DIR" ]; then
    ...
    warn "No OhMyAgentTeam installation found at $INSTALL_DIR"
  fi

  if command_exists omat; then
    omat daemon stop 2>/dev/null && ok "Daemon stopped" || true
  fi
  ...
  while [ $# -gt 0 ]; do
    case "$1" in
      ...
      --help|-h)
        ...
        echo "  OMAT_INSTALL_DIR   Self-host server install directory"
        echo "                        (default: \$HOME/.ohmyagentteam/server)"
        echo "  OMAT_BIN_DIR       Target directory for the CLI binary when"
        echo "                        installing from GitHub Releases"
        echo "                        (default: /usr/local/bin, then \$HOME/.local/bin)"
        echo "  OMAT_SELFHOST_REF  Git ref to check out for self-host assets"
        echo "                        (default: latest release tag, falling back to main)"
        echo ""
        echo "After installation, run 'omat setup' to configure your environment."
        exit 0
        ;;
