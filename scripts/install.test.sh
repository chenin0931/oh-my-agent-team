...
#!/usr/bin/env bash
if [[ "$*" == *"-sI"* ]]; then
  printf 'HTTP/2 302\r\nlocation: https://github.com/chenin0931/oh-my-agent-team/releases/tag/v0.3.2\r\n'
  exit 0
fi
...
_run_installer() {
  ...
  if [[ ! -x "$tmp/install-bin/omat" ]]; then
    echo "expected fallback binary at $tmp/install-bin/omat" >&2
    cat "$out" >&2 || true
    cat "$err" >&2 || true
    return 1
  fi
