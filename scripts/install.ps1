# OhMyAgentTeam installer for Windows — one command to get started.
#
# Install CLI (default): connects to ohmyagentteam.com
#   irm https://raw.githubusercontent.com/chenin0931/oh-my-agent-team/main/scripts/install.ps1 | iex
#
# Self-host: starts a local OhMyAgentTeam server + installs CLI + configures
#   $env:OMAT_MODE="local"; irm https://raw.githubusercontent.com/chenin0931/oh-my-agent-team/main/scripts/install.ps1 | iex
#
...
# Configuration
# ---------------------------------------------------------------------------
$RepoUrl       = "https://github.com/chenin0931/oh-my-agent-team.git"
$RepoWebUrl    = "https://github.com/chenin0931/oh-my-agent-team"
$DefaultInstallDir = Join-Path $env:USERPROFILE ".ohmyagentteam\server"
$InstallDir    = if ($env:OMAT_INSTALL_DIR) { $env:OMAT_INSTALL_DIR } else { $DefaultInstallDir }
...
function Get-LatestVersion {
    try {
        $release = Invoke-RestMethod -Uri "https://api.github.com/repos/chenin0931/oh-my-agent-team/releases/latest" -ErrorAction Stop
...
function Get-SelfHostRef {
    if ($env:OMAT_SELFHOST_REF) {
        return $env:OMAT_SELFHOST_REF
    }
...
function Get-InstalledCliVersion {
    try {
        $firstLine = omat version 2>$null | Select-Object -First 1
...
function Install-CliBinary {
    Write-Info "Installing OhMyAgentTeam CLI from GitHub Releases..."

    if (-not [Environment]::Is64BitOperatingSystem) {
        Write-Fail "OhMyAgentTeam requires a 64-bit Windows installation."
    }
    ...
    $url = "https://github.com/chenin0931/oh-my-agent-team/releases/download/$latest/omat-cli-$version-windows-$arch.zip"
    $tmpDir = Join-Path ([System.IO.Path]::GetTempPath()) "omat-install"
    ...
    try {
        Invoke-WebRequest -Uri $url -OutFile (Join-Path $tmpDir "omat.zip") -UseBasicParsing
    } catch {
    ...
    $checksumUrl = "https://github.com/chenin0931/oh-my-agent-team/releases/download/$latest/checksums.txt"
    try {
        ...
        $zipFile = Join-Path $tmpDir "omat.zip"
        $actualHash = (Get-FileHash -Path $zipFile -Algorithm SHA256).Hash.ToLower()
        $releaseAsset = "omat-cli-$version-windows-$arch.zip"
    ...
    Expand-Archive -Path (Join-Path $tmpDir "omat.zip") -DestinationPath $tmpDir -Force

    $binDir = Join-Path $env:USERPROFILE ".ohmyagentteam\bin"
    if (-not (Test-Path $binDir)) {
        New-Item -ItemType Directory -Path $binDir -Force | Out-Null
    }

    $exeSrc = Join-Path $tmpDir "omat.exe"
    if (-not (Test-Path $exeSrc)) {
        $exeSrc = Get-ChildItem -Path $tmpDir -Filter "omat.exe" -Recurse | Select-Object -First 1 -ExpandProperty FullName
    }
    if (-not $exeSrc -or -not (Test-Path $exeSrc)) {
        Remove-Item $tmpDir -Recurse -Force
        Write-Fail "omat.exe not found in downloaded archive."
    }

    Copy-Item $exeSrc (Join-Path $binDir "omat.exe") -Force
    Remove-Item $tmpDir -Recurse -Force

    Add-ToUserPath $binDir
    Write-Ok "OhMyAgentTeam CLI installed to $binDir\omat.exe"
}
...
function Install-Cli {
    if (Test-CommandExists "omat") {
        ...
        if ($isUpToDate) {
            Write-Ok "OhMyAgentTeam CLI is up to date ($currentVer)"
            return
        }

        Write-Info "OhMyAgentTeam CLI $currentVer installed, latest is $latestVer - upgrading..."
        Install-CliBinary

        $newVer = Get-InstalledCliVersion
        Write-Ok "OhMyAgentTeam CLI upgraded ($currentVer -> $newVer)"
        return
    }

    Install-CliBinary

    if (-not (Test-CommandExists "omat")) {
        Write-Fail "CLI installed but 'omat' not found on PATH. Restart your terminal and try again."
    }
}
...
function Test-Docker {
    if (-not (Test-CommandExists "docker")) {
        Write-Fail @"
Docker is not installed. OhMyAgentTeam self-hosting requires Docker and Docker Compose.

Install Docker Desktop for Windows:
  https://docs.docker.com/desktop/install/windows-install/

After installing Docker, re-run this script with `$env:OMAT_MODE="local"`.
"@
    }
...
function Install-Server {
    Write-Info "Setting up OhMyAgentTeam server..."
    ...
    if (Test-Path (Join-Path $InstallDir ".git")) {
        ...
        Write-Info "Cloning OhMyAgentTeam repository..."
    ...
    Write-Info "Pulling official OhMyAgentTeam images..."
    Pull-OfficialSelfHostImages
    Write-Info "Starting OhMyAgentTeam services (this may take a few minutes on first run)..."
    ...
    if ($ready) {
        Write-Ok "OhMyAgentTeam server is running"
    } else {
...
function Start-DefaultInstall {
    Write-Host ""
    Write-Host "  OhMyAgentTeam - Installer" -ForegroundColor White
    Write-Host ""
    ...
    Write-Host "  [OK] OhMyAgentTeam CLI is ready!" -ForegroundColor Green
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host ""
    Write-Host "  Next: configure your environment"
    Write-Host ""
    Write-Host "     omat setup                  " -NoNewline; Write-Host "# Connect this computer" -ForegroundColor DarkGray
    Write-Host "     omat setup self-host        " -NoNewline; Write-Host "# Connect to a self-hosted server" -ForegroundColor DarkGray
    Write-Host ""
    Write-Host "  Self-hosting? Install the server first:"
    Write-Host '     $env:OMAT_MODE="with-server"; irm https://raw.githubusercontent.com/chenin0931/oh-my-agent-team/main/scripts/install.ps1 | iex'
    Write-Host ""
}
...
function Start-LocalInstall {
    Write-Host ""
    Write-Host "  OhMyAgentTeam - Self-Host Installer" -ForegroundColor White
    ...
    Write-Host ""
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host "  [OK] OhMyAgentTeam server is running and CLI is ready!" -ForegroundColor Green
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "     omat setup self-host     " -NoNewline; Write-Host "# Configure + authenticate + start daemon" -ForegroundColor DarkGray
    Write-Host ""
    ...
    Write-Host '     $env:OMAT_MODE="stop"; irm https://raw.githubusercontent.com/chenin0931/oh-my-agent-team/main/scripts/install.ps1 | iex'
    Write-Host ""
}
...
function Start-Stop {
    Write-Host ""
    Write-Info "Stopping OhMyAgentTeam services..."
    ...
    if (Test-Path $InstallDir) {
        ...
        Write-Warn "No OhMyAgentTeam installation found at $InstallDir"
    }

    if (Test-CommandExists "omat") {
        try {
            omat daemon stop 2>$null
...
# Entry point
# ---------------------------------------------------------------------------
$mode = if ($env:OMAT_MODE) { $env:OMAT_MODE.ToLower() } else { "default" }
