...
} from "@ohmyagentteam/core/inbox/mutations";

import { IssueDetailEntry } from "../../issues/components";
import { EpicDetail } from "../../epics/components";
...
export function InboxPage() {
  ...
  const urlTargetType = searchParams.get("type") === "epic" ? "epic" : "issue";
  const wsPaths = useWorkspacePaths();

  const [selectedKey, setSelectedKeyState] = useState(() => urlIssue);
  const [selectedKeyType, setSelectedKeyType] = useState<"issue" | "epic">(
    () => urlTargetType,
  );
  ...
  useEffect(() => {
    ...
    setSelectedKeyType(urlTargetType);
  }, [urlIssue, urlTargetType]);

  const wsId = useWorkspaceId();
  const { data: rawItems = [], isLoading: loading } = useQuery(inboxListOptions(wsId));
  const items = useMemo(() => deduplicateInboxItems(rawItems), [rawItems]);
  const [view, setView] = useState<InboxView>("all");
  const viewCounts = useMemo(() => countInboxViews(items), [items]);
  const visibleItems = useMemo(
    () => items.filter((item) => matchesInboxView(item, view)),
    [items, view],
  );

  const selected = items.find((item) => {
    const id = item.target_id ?? item.issue_id ?? item.id;
    const type = item.target_type ?? (item.issue_id ? "issue" : null);
    return id === selectedKey && (!item.target_id || type === selectedKeyType);
  }) ?? null;
  ...
  const setSelectedKey = useCallback((key: string, targetType: "issue" | "epic" = "issue") => {
    setSelectedKeyState(key);
    setSelectedKeyType(targetType);
    const inboxPath = wsPaths.inbox();
    const url = key ? `${inboxPath}?issue=${key}${targetType === "epic" ? "&type=epic" : ""}` : inboxPath;
    replace(url);
  }, [replace, wsPaths]);

  // Keep the split-view detail consistent with the active filter. Selecting
  // the first visible row automatically would also mark it read, so clear the
  // detail and let the user choose deliberately instead.
  useEffect(() => {
    if (!selectedKey || !selected) return;
    const isVisible = visibleItems.some(
      (item) => (item.target_id ?? item.issue_id ?? item.id) === selectedKey,
    );
    if (!isVisible) setSelectedKey("");
  }, [selectedKey, selected, visibleItems, setSelectedKey]);
  ...
  useEffect(() => {
    ...
    replace(selectedKeyType === "epic" ? wsPaths.epicDetail(selectedKey) : wsPaths.issueDetail(selectedKey));
  }, [loading, selectedKey, selected, replace, wsPaths, setSelectedKey, selectedKeyType]);

  const { defaultLayout, onLayoutChanged } = useDefaultLayout({
    id: "omat_inbox_layout",
  });
  ...
  const handleSelect = (item: InboxItem) => {
    setSelectedKey(
      item.target_id ?? item.issue_id ?? item.id,
      item.target_type ?? (item.issue_id ? "issue" : "issue"),
    );
  };
  ...
  const handleArchive = (id: string) => {
    ...
    const wasSelected =
      !!archived && (archived.target_id ?? archived.issue_id ?? archived.id) === selectedKey;
    if (wasSelected) {
      ...
      setSelectedKey(
        next ? (next.target_id ?? next.issue_id ?? next.id) : "",
        next?.target_type ?? "issue",
      );
    }
  ...
  const handleArchiveAllRead = () => {
    const readKeys = items.filter((i) => i.read).map((i) => i.target_id ?? i.issue_id ?? i.id);
    ...
    <div className="grid h-10 shrink-0 grid-cols-4 gap-0.5 border-b px-1" role="tablist" aria-label={t(($) => $.filters.label)}>
      {([
        ...
        >
          ...
          <span className="min-w-0 truncate">{t(($) => $.filters[id])}</span>
          {viewCounts[id] > 0 && (
            <span className="hidden shrink-0 text-[10px] tabular-nums text-muted-foreground @xl:inline">
              {viewCounts[id]}
            </span>
          )}
        </button>
  ...
  const listBody = visibleItems.length === 0 ? (
    <div className="flex flex-col items-center justify-center py-16 text-muted-foreground">
      ...
      <p className="text-sm">{t(($) => view === "all" ? $.list.empty : $.list.empty_filter)}</p>
    </div>
  ) : (
    <div>
      {visibleItems.map((item) => (
        <InboxListItem
          key={item.id}
          item={item}
          isSelected={(item.target_id ?? item.issue_id ?? item.id) === selectedKey}
  ...
  const selectedTargetId = selected?.target_id ?? selected?.issue_id ?? null;
  const selectedTargetType = selected?.target_type ?? (selected?.issue_id ? "issue" : null);
  const detailContent = selectedTargetId && selectedTargetType === "epic" ? (
    <ErrorBoundary resetKeys={[selectedTargetId]}>
      <EpicDetail
        key={selectedTargetId}
        epicId={selectedTargetId}
        onDone={() => { if (selected) handleArchive(selected.id); }}
      />
    </ErrorBoundary>
  ) : selectedTargetId && selectedTargetType === "issue" ? (
    ...
    <ErrorBoundary resetKeys={[selectedTargetId]}>
      <IssueDetailEntry
        key={selectedTargetId}
        issueId={selectedTargetId}
        defaultSidebarOpen={false}
        layoutId="omat_inbox_issue_detail_layout"
  ...
  if (isMobile) {
    ...
    return (
      <div className="flex flex-1 flex-col min-h-0 @container">
        ...
        {viewTabs}
        <div className="flex-1 min-h-0 overflow-y-auto">
          <div className="pb-20">{listBody}</div>
        </div>
      </div>
    );
  }
