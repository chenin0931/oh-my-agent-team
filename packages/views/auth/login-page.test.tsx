...
vi.mock("@ohmyagentteam/core/types", () => ({}));
...
describe("LoginPage", () => {
  ...
  it("renders email form with 'Sign in to OhMyAgentTeam' title", () => {
    renderWithI18n(<LoginPage onSuccess={onSuccess} />);
    expect(
      screen.getByText(/sign in to ohmyagentteam/i),
    ).toBeInTheDocument();
    expect(
  ...
  it("shows cli_confirm step when existing session + cliCallback", async () => {
    localStorage.setItem("omat_token", "existing-jwt");
