...
const workspaceRef = vi.hoisted(() => ({
  current: {
    ...
    name: "Test Workspace",
    slug: "test-workspace",
    repos: [{ url: "https://github.com/chenin0931/oh-my-agent-team" }] as { url: string; description?: string }[],
  },
}));
...
describe("RepositoriesTab — view/edit toggle", () => {
  beforeEach(() => {
    ...
    workspaceRef.current = {
      ...
      name: "Test Workspace",
      slug: "test-workspace",
      repos: [{ url: "https://github.com/chenin0931/oh-my-agent-team" }],
    };
  ...
  it("renders persisted repos in display mode (no input)", () => {
    ...
    expect(screen.getByText("https://github.com/chenin0931/oh-my-agent-team")).toBeTruthy();
  });
  ...
  it("clicking Edit reveals an input pre-filled with the URL", async () => {
    ...
    expect(inputs[0]!.value).toBe("https://github.com/chenin0931/oh-my-agent-team");
  });
  ...
  it("Save re-enables after editing, then returns to display mode + disabled on success", async () => {
    ...
    await user.type(input, "https://github.com/chenin0931/edited");
  ...
  it("Cancel on a dirty edited row reverts the URL and exits edit mode", async () => {
    ...
    await user.clear(input);
    await user.type(input, "https://github.com/chenin0931/changed");
    expect(screen.getByRole("button", { name: /^Save$/ })).not.toBeDisabled();
  ...
  it("accepts scp-like shorthand without browser URL validation blocking submit", async () => {
    ...
    const input = screen.getAllByRole("textbox")[0] as HTMLInputElement;
    await user.clear(input);
    await user.type(input, "git@github.com:chenin0931/oh-my-agent-team.git");
    ...
    await waitFor(() => {
      expect(mockUpdateWorkspace).toHaveBeenCalledWith("workspace-1", {
        repos: [{ url: "git@github.com:chenin0931/oh-my-agent-team.git" }],
      });
    });
  });
  ...
  it("description field is editable and included in save payload", async () => {
    workspaceRef.current = {
      ...workspaceRef.current,
      repos: [{ url: "https://github.com/chenin0931/oh-my-agent-team", description: "Main app" }],
    };
    const user = userEvent.setup();
    mockUpdateWorkspace.mockImplementation(
      async (_id: string, payload: { repos: { url: string; description?: string }[] }) => {
    ...
    await waitFor(() => {
      expect(mockUpdateWorkspace).toHaveBeenCalledWith("workspace-1", {
        repos: [{ url: "https://github.com/chenin0931/oh-my-agent-team", description: "Updated description" }],
      });
    });
  });
});
