...
This guide documents the local development workflow for contributors working on the OhMyAgentTeam codebase.
...
- the main checkout usually uses `.env` and `POSTGRES_DB=ohmyagentteam`
...
```bash
POSTGRES_DB=ohmyagentteam
POSTGRES_PORT=5432
DATABASE_URL=postgres://ohmyagentteam:ohmyagentteam@localhost:5432/ohmyagentteam?sslmode=disable
PORT=8080
FRONTEND_PORT=3000
```
...
```bash
POSTGRES_DB=omat_my_feature_702
POSTGRES_PORT=5432
PORT=18782
FRONTEND_PORT=13702
DATABASE_URL=postgres://ohmyagentteam:ohmyagentteam@localhost:5432/omat_my_feature_702?sslmode=disable
```
...
```bash
git worktree add ../ohmyagentteam-feature -b feat/my-change main
cd ../ohmyagentteam-feature
make dev
```
...
- main checkout
  - database: `ohmyagentteam`
  - backend: `8080`
  - frontend: `3000`
- worktree checkout
  - database: `omat_my_feature_702`
...
The daemon authenticates using the CLI's stored token (`omat login`).
...
whatever server is configured in `~/.ohmyagentteam/config.json`. That's fine for
...
Example: worktree at `../ohmyagentteam-feat-auth` produces profile
`dev-omat_feat_auth-347`, matching that worktree's port and database
allocation.
...
For deterministic local automation, set `OMAT_DEV_VERIFICATION_CODE=888888`
...
CONFIG_DIR="$HOME/.ohmyagentteam/profiles/$PROFILE"
mkdir -p "$CONFIG_DIR"
...
local backend. Agent-executed `ohmyagentteam` commands automatically use the same
...
rm -rf "$HOME/.ohmyagentteam/profiles/$PROFILE"
```
...
1. Compiles the `ohmyagentteam` CLI from `server/cmd/omat` into
   `apps/desktop/resources/bin/omat`
...
backend logs. If you set `OMAT_DEV_VERIFICATION_CODE=888888` before starting
...
- `DESKTOP_APP_SUFFIX` = `<folder>-<offset>` — its own single-instance lock /
  `userData`, and an app named `OhMyAgentTeam Canary <folder>-<offset>` so it is
  distinguishable in Cmd+Tab. The offset keeps it unique across worktrees that
  share a folder name at different paths.

The primary checkout is left untouched (`5173`, `OhMyAgentTeam Canary`). Set either
...
Nothing in this flow touches the system-installed `ohmyagentteam` or the default
`~/.ohmyagentteam/config.json`:

| Resource | System / Production | Local Dev (per-worktree) |
|---|---|---|
| Config | `~/.ohmyagentteam/config.json` | `~/.ohmyagentteam/profiles/dev-<slug>-<hash>/config.json` |
| Daemon PID | `~/.ohmyagentteam/daemon.pid` | `~/.ohmyagentteam/profiles/dev-<slug>-<hash>/daemon.pid` |
| Health port | `19514` | `19514 + 1 + (name_hash % 1000)` |
| Workspaces dir | `~/omat_workspaces/` | `~/omat_workspaces_dev-<slug>-<hash>/` |
| Database | remote / production | local Docker: `omat_<slug>_<hash>` |
| Desktop profile | `desktop-api.ohmyagentteam.com` | `desktop-localhost-<port>` |
...
```bash
docker compose exec -T postgres psql -U ohmyagentteam -d postgres -At -c "select datname from pg_database order by datname;"
```
